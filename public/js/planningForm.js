...
async function getLastFieldVueSeance() {
  const urlAxios = `/projetsForm/getLastFieldVueSeances`;
...
async function getFirstNameModules() {
  const url = '/cfp/modules/getAllModule';
...
{
  let donnee;
  const url = `/projetsForm/${idProjet}/detailForm`;
...
function updateEventMoved(args) {
  ...
  const url = `/projetsForm/${idSeance}/update`;
  fetch(url, {
...
function deleteEvent(args) {
  ...
  const url = `/projetsForm/${idSeance}/delete`;
  const token = getToken();
      ...
      fetch(
        ...
        getLastFieldVueSeance().then(seance => {
          getIdNamePaiementStatusEtp(seance.idProjet).then(object => {
            ...
            const newDataInStoreEventsResources = [...lastDataInStoreEventsResources, ...oneEvent];
            this.setStore('ACCESS_EVENTS_RESOURCE_' + idCustomer, newDataInStoreEventsResources);
