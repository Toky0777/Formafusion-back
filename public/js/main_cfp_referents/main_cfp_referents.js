function addReferent() {
  $.ajax({
    ...
    data: {
      ...
      emp_fonction: $('#main_ref_fonction').val()
    },
    dataType: "json",
    beforeSend: function () {
...
function mainClearRefFields() {
  $('#main_ref_matricule').val('');
  $('#main_ref_name').val('');
  $('#main_ref_firstname').val('');
  $('#main_ref_email').val('');
  $('#main_ref_phone').val('');
  $('#main_ref_fonction').val('');
}

function mainEditReferent(idEmploye) {
  $.ajax({
    type: "get",
    url: "/cfp/referents/" + idEmploye + "/edit",
    dataType: "json",
    success: function (res) {
      ...
      if (res.emp.photo != null) {
        photoReferent.append(`<img class="w-24 h-24 object-cover rounded-full" src="${endpoint}/${bucket}/img/referents/` + res.emp.photo + `" alt="" width="50px" height="50px">`);
      } else {
