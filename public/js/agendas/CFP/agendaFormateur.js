document.addEventListener('DOMContentLoaded', (e) => {
  ...
  const prevLink = document.getElementById('prevLinkForm');
  const nextLink = document.getElementById('nextLinkForm');
  const todayLink = document.getElementById('todayLinkForm');

  prevLink.addEventListener('click', function (e) {
    e.preventDefault(); // Empêcher le comportement par défaut du lien
    const monthDaypilot = dpFormateurs.startDate.value;
    const date = new Date(monthDaypilot);
    const month = date.getUTCMonth();
    document.getElementById('monthSelectorWeekForm').options[month].selected = true;

    dpFormateurs.startDate = dpFormateurs.startDate.addDays(-7);
    dpFormateurs.update();
  });
  ...
  todayLink.addEventListener('click', function (e) {
    ...
    dpFormateurs.startDate = DayPilot.Date.today();
    dpFormateurs.update();
...
document.querySelectorAll("select").forEach(function(el){
  el.addEventListener("change",function(){
    ...
    dpFormateurs.startDate = DayPilot.Date.today();
    dpFormateurs.startDate = dpFormateurs.startDate.addMonths(selectedMonth-1);
    dpFormateurs.update();
  })
}) 
...
const dpFormateurs = new DayPilot.Scheduler("dpFormateurs", {
  ...
  onEventClick: args => {
    ...
    const idProjet = DayPilot.Util.escapeHtml(e.data.idProjet);
    console.log("CLIC...",e);
    dpFormateurs.clearSelection();
    dpFormateurs.update(e);
  },
  ...
  bubble: new DayPilot.Bubble({
    onLoad: (args) => {
      const e = args.source;
      console.log('quartier-->', e.data.quartier)
                ...
                <div class="flex flex-col">
                  ...
                  <p class="text-base text-gray-400">Projet : <span
                      class="text-base text-white px-2 rounded-md bg-[${colors}] "> ${typeProjet || 'non assigné'}</span>
                           ...
                           <p class="text-sm text-gray-400"> Modalite : <span
...
const formateurs = {
  ...
  getEventData() {
    ...
    //let objectEvents = resourceEvents ? resourceEvents : this.getAllResourceEvents();
    const objectEvents =  resourceEvents;//<== àcompleter ;

    
      for (const evnt of objectEvents || []) {
        // Ajout sur les lignes formateurs ...  
        // console.log('evnt OBJ-->',evnt)
        evnt.prenom_form.forEach(form => {
          formateurs.push({
            ...
            html: `<div class="flex items-start gap-2 bg-white p-1 w-full h-full mt-1">
...
const appFormElements = {
  elements: {
    ...
    tousF: document.querySelector("#tousFormateur"),
    subdropForm: document.querySelector("#subdropFormFormateur")

  },

  init() {
    this.elements.clearF.addEventListener("click", handleClearAction);
    this.elements.tousF.addEventListener("click", handleClearAction);
  },
