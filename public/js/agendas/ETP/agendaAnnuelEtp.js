document.addEventListener('DOMContentLoaded', (e) => {
  ...
  formateurs.getResourceEtpFormateurs();

  formateurs.loadAllEventsEtpFormateurs();
  ...
  tabSelectorAnnuelle.addEventListener('change', function (e) {
    ...
    console.log('Change ANNUAIRE...', selectedContentId);
    hideForm();
  ...
  tabSelectorCours.addEventListener('change', function (e) {
    const selectedContentId = `${this.value}`;
    hideForm();
    displayForm(selectedContentId);
    setSelectTab(selectedContentId);
  ...
  tabSelectorFormateur.addEventListener('change', function (e) {
    const selectedContentId = `${this.value}`;
    console.log('Change FORMS', selectedContentId);
    hideForm();
    displayForm(selectedContentId);
    setSelectTab(selectedContentId);
  });

  // tabSelectorClient.addEventListener('change', function (e) {
  //   const selectedContentId = `${this.value}`;
  //   hideForm();
  //   displayForm(selectedContentId);
  //   setSelectTab(selectedContentId);
...
const dpAnnuaire = new DayPilot.Scheduler("dpAnnuaire", {
  ...
  bubble: new DayPilot.Bubble({
    onLoad: (args) => {
      ...
      console.log('NB_APPR-->',e.data.nb_appr);
  ...
  onScroll: async args => {
    ...
    args.events = appAnnuel.getEventData();  // charge sessionStorage("EVENTS_DETAILS")

    // getAllSeancesRessourceJson();            // charge sessionStorage("EVENTS_RESOURCE")

    //  console.log('appAnnuel-->',args.events);
    args.loaded();
  ...
  getAllDetailEvents() {
    ...
    fetch(url, { method: "GET" }).then(response => response.json())
      .then(data => {
        ...
        console.log('LISTE SEANCES ETP... LOADING...objectEvents-->', objectEvents);
        let idCustomer;
  ...
  getAllEventsGroupBy() {
   // dpAnnuaire.init();
    ...
    fetch(url, { method: "GET" }).then(response => response.json())
      .then(data => {
        const objectEvents = (data.seances) ? data.seances : [];
        console.log('LISTE SEANCES LOADING...GROUP BY objectEvents-->', objectEvents);
        let idCustomer;
        if (sessionStorage.getItem('ID_CUSTOMER') !== null) {
          idCustomer = sessionStorage.getItem('ID_CUSTOMER');
        }
