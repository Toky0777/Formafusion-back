...
var FullCalendar = (function (exports) {
    ...
    */
    ...
    function listenToHoverBySelector(container, selector, onMouseEnter, onMouseLeave) {
        ...
        return listenBySelector(container, 'mouseover', selector, (mouseOverEv, matchedChild) => {
            if (matchedChild !== currentMatchedChild) {
                ...
                const realOnMouseLeave = (mouseLeaveEv) => {
    ...
    class NativeFormatter {
        constructor(formatSettings) {
            ...
            let severity = 0;
            for (const name in formatSettings) {
        ...
        formatRange(start, end, context, betterDefaultSeparator) {
            ...
            const separator = extendedSettings.separator || betterDefaultSeparator || context.defaultSeparator || '';
            if (insertion) {
    ...
    function buildNativeFormattingFunc(standardDateProps, extendedSettings, context) {
        ...
        if (extendedSettings.omitZeroMinute) {
            const zeroProps = Object.assign({}, standardDateProps);
    ...
    */
    ...
    function refineProps(input, refiners) {
        const refined = {};
        const extra = {};
        for (const propName in refiners) {
    ...
    function mergeProps(propObjs, complexPropsMap) {
        const dest = {};
        if (complexPropsMap) {
            for (const name in complexPropsMap) {
                if (complexPropsMap[name] === isMaybeObjectsEqual) { // implies that it's object-mergeable
                    const complexObjs = [];
                    // collect the trailing object values, stopping when a non-object is discovered
                    for (let i = propObjs.length - 1; i >= 0; i -= 1) {
                        const val = propObjs[i][name];
    ...
    */
    function flushSync(runBeforeFlush) {
        runBeforeFlush();
        const oldDebounceRendering = l$1.debounceRendering; // orig
        const callbackQ = [];
    ...
    function createContext(defaultValue) {
        const ContextType = G$1(defaultValue);
    ...
    class ContentInjector extends BaseComponent {
        ...
        applyQueueudDomNodes() {
            ...
            if (!isArraysEqual(queuedDomNodes, currentDomNodes)) {
                ...
                for (const newNode of queuedDomNodes) {
    ...
    */
    ...
    class ViewContainer extends BaseComponent {
        render() {
            const { props, context } = this;
            const { options } = context;
            const renderProps = { view: context.viewApi };
    ...
    class DateProfileGenerator {
        ...
        buildValidRange() {
            ...
            const simpleInput = typeof input === 'function'
        ...
        adjustActiveRange(range) {
            const { dateEnv, usesMinMaxTime, slotMinTime, slotMaxTime } = this.props;
            let { start, end } = range;
            if (usesMinMaxTime) {
        ...
        buildRangeFromDuration(date, direction, duration, unit) {
            ...
            if (!dateAlignment) {
                const { dateIncrement } = this.props;
                if (dateIncrement) {
        ...
        buildRangeFromDayCount(date, direction, dayCount) {
            const { dateEnv, dateAlignment } = this.props;
            let runningCount = 0;
            let start = date;
            let end;
            if (dateAlignment) {
    ...
    function expandRecurring(eventStore, framingRange, context) {
        const { dateEnv, pluginHooks, options } = context;
        ...
        for (const defId in defs) {
            ...
            if (def.recurringDef) {
                ...
                const starts = expandRecurringRanges(def, duration, framingRange, dateEnv, pluginHooks.recurringTypes);
    ...
    */
    ...
    function parseEvent(raw, eventSource, context, allowOpenRange, refiners = buildEventRefiners(context), defIdMap, instanceIdMap) {
        const { refined, extra } = refineEventDef(raw, context, refiners);
        const defaultAllDay = computeIsDefaultAllDay(eventSource, context);
        const recurringRes = parseRecurring(refined, defaultAllDay, context.dateEnv, context.pluginHooks.recurringTypes);
        if (recurringRes) {
            const def = parseEventDef(refined, extra, eventSource ? eventSource.sourceId : '', recurringRes.allDay, Boolean(recurringRes.duration), context, defIdMap);
            def.recurringDef = {
        ...
        const singleRes = parseSingle(refined, defaultAllDay, context, allowOpenRange);
        if (singleRes) {
            const def = parseEventDef(refined, extra, eventSource ? eventSource.sourceId : '', singleRes.allDay, singleRes.hasEnd, context, defIdMap);
    ...
    */
    function parseEventDef(refined, extra, sourceId, allDay, hasEnd, context, defIdMap) {
        ...
        for (const memberAdder of context.pluginHooks.eventDefMemberAdders) {
    ...
    function parseEvents(rawEvents, eventSource, context, allowOpenRange, defIdMap, instanceIdMap) {
        ...
        for (const rawEvent of rawEvents) {
            const tuple = parseEvent(rawEvent, eventSource, context, allowOpenRange, eventRefiners, defIdMap, instanceIdMap);
            if (tuple) {
    ...
    function getRelevantEvents(eventStore, instanceId) {
        ...
        if (instance) {
            ...
            const newStore = filterEventStoreDefs(eventStore, (lookDef) => isEventDefsGrouped(def, lookDef));
            // add the original
    ...
    function filterEventStoreDefs(eventStore, filterFunc) {
        ...
        const instances = filterHash(eventStore.instances, (instance) => (defs[instance.defId] // still exists?
        ));
        return { defs, instances };
    }
    function excludeSubEventStore(master, sub) {
        const { defs, instances } = master;
        const filteredDefs = {};
    ...
    function parseEventSource(raw, context, refiners = buildEventSourceRefiners(context)) {
        ...
        if (rawObj) {
            const { refined, extra } = refineProps(rawObj, refiners);
    ...
    function resetRawEvents(existingEventStore, eventSource, rawEvents, activeRange, context) {
        ...
        const newEventStore = parseEvents(transformRawEvents(rawEvents, eventSource, context), eventSource, context, false, defIdMap, instanceIdMap);
    ...
    function rezoneEventStoreDates(eventStore, oldDateEnv, newDateEnv) {
        const { defs } = eventStore;
        const instances = mapHash(eventStore.instances, (instance) => {
    ...
    */
    ...
    function buildDateSpanApiWithContext(dateSpan, context) {
        const props = {};
        for (const transform of context.pluginHooks.dateSpanTransforms) {
    ...
    function applyMutationToEventStore(eventStore, eventConfigBase, mutation, context) {
        ...
        for (const defId in eventStore.defs) {
            const def = eventStore.defs[defId];
            dest.defs[defId] = applyMutationToEventDef(def, eventConfigs[defId], mutation, context);
        }
        for (const instanceId in eventStore.instances) {
            const instance = eventStore.instances[instanceId];
            const def = dest.defs[instance.defId]; // important to grab the newly modified def
    ...
    function applyMutationToEventDef(eventDef, eventConfig, mutation, context) {
        ...
        const copy = Object.assign(Object.assign(Object.assign({}, eventDef), standardProps), { ui: Object.assign(Object.assign({}, eventDef.ui), standardProps.ui) });
        if (mutation.extendedProps) {
            copy.extendedProps = Object.assign(Object.assign({}, copy.extendedProps), mutation.extendedProps);
        }
        for (const applier of context.pluginHooks.eventDefMutationAppliers) {
    ...
    eventConfig, mutation, context) {
        ...
        const copy = Object.assign({}, eventInstance);
        if (forceAllDay) {
        ...
        */
        ...
        mutate(mutation) {
            ...
            if (instance) {
                ...
                const { eventStore } = context.getCurrentData();
                ...
                const oldEvent = new EventImpl(context, def, instance); // snapshot
    ...
    */
    function sliceEventStore(eventStore, eventUiBases, framingRange, nextDayThreshold) {
        ...
        for (const groupId in inverseBgByGroupId) { // BY GROUP
            const ranges = inverseBgByGroupId[groupId];
            const invertedRanges = invertRanges(ranges, framingRange);
            for (const invertedRange of invertedRanges) {
                const def = defByGroupId[groupId];
                const ui = eventUis[def.defId];
                bgRanges.push({
                    def,
                    ui,
        ...
        for (const defId in inverseBgByDefId) {
            const ranges = inverseBgByDefId[defId];
            const invertedRanges = invertRanges(ranges, framingRange);
            for (const invertedRange of invertedRanges) {
                bgRanges.push({
    ...
    function buildSegCompareObj(seg) {
        ...
        const range = eventRange.instance ? eventRange.instance.range : eventRange.range;
    ...
    startOverride, endOverride) {
        ...
        const wholeEventStart = eventInstance.range.start;
        const wholeEventEnd = eventInstance.range.end;
        let segStart = startOverride || seg.start || seg.eventRange.range.start;
        let segEnd = endOverride || seg.end || seg.eventRange.range.end;
        const isStartDay = startOfDay(wholeEventStart).valueOf() === startOfDay(segStart).valueOf();
        const isEndDay = startOfDay(addMs(wholeEventEnd, -1)).valueOf() === startOfDay(addMs(segEnd, -1)).valueOf();
    ...
    function getSegAnchorAttrs(seg, context) {
        const { def, instance } = seg.eventRange;
        const { url } = def;
        if (url) {
            return { href: url };
        }
    ...
    */
    function parseOpenDateSpan(raw, dateEnv) {
        const { refined: standardProps, extra } = refineProps(raw, STANDARD_PROPS);
    ...
    function fabricateEventRange(dateSpan, eventUiBases, context) {
        ...
        const def = parseEventDef(res.refined, res.extra, '', // sourceId
    ...
    */
    function unpromisify(func, normalizedSuccessCallback, normalizedFailureCallback) {
        ...
        const wrappedFailure = function (error) {
            if (!isResolved) {
                isResolved = true;
    ...
    class CalendarRoot extends BaseComponent {
        ...
        render() {
            const { props } = this;
            const { options } = props;
            const { forPrint } = this.state;
            const isHeightAuto = forPrint || options.height === 'auto' || options.contentHeight === 'auto';
            const height = (!isHeightAuto && options.height != null) ? options.height : '';
            const classNames = [
                'fc',
    ...
    class CalendarImpl {
            ...
            this.batchRendering(() => {
                ...
                if (dateOrRange) {
                    if (dateOrRange.start && dateOrRange.end) { // a range
                        ...
                        const { dateEnv } = this.getCurrentData();
                        this.dispatch({
                            type: 'CHANGE_VIEW_TYPE',
                            viewType,
        ...
        getUnitViewSpec(unit) {
            const { viewSpecs, toolbarConfig } = this.getCurrentData();
            const viewTypes = [].concat(toolbarConfig.header ? toolbarConfig.header.viewsWithButtons : [], toolbarConfig.footer ? toolbarConfig.footer.viewsWithButtons : []);
            let i;
            let spec;
        ...
        select(dateOrObj, endDate) {
            ...
            const state = this.getCurrentData();
            const selection = parseDateSpan(selectionInput, state.dateEnv, createDuration({ days: 1 }));
        ...
        addEvent(eventInput, sourceInput) {
            ...
            if (tuple) {
                const newEventApi = new EventImpl(state, tuple.def, tuple.def.recurringDef ? null : tuple.instance);
                this.dispatch({
                    type: 'ADD_EVENTS',
        ...
        triggerEventAdd(eventApi) {
            const { emitter } = this.getCurrentData();
        ...
        getEventById(id) {
            const state = this.getCurrentData();
            const { defs, instances } = state.eventStore;
            id = String(id);
            for (const defId in defs) {
                const def = defs[defId];
                if (def.publicId === id) {
    ...
    class Splitter {
        ...
        _splitEventStore(eventStore, defKeys) {
            ...
            const splitStores = {};
            for (const defId in defs) {
                for (const key of defKeys[defId]) {
                    if (!splitStores[key]) {
            ...
            for (const instanceId in instances) {
                const instance = instances[instanceId];
                for (const key of defKeys[instance.defId]) {
        ...
        _splitInteraction(interaction) {
            const splitStates = {};
            if (interaction) {
                const affectedStores = this._splitEventStore(interaction.affectedEvents, this._getKeysForEventDefs(interaction.affectedEvents));
                // can't rely on defKeys because event data is mutated
                const mutatedKeysByDefId = this._getKeysForEventDefs(interaction.mutatedEvents);
                const mutatedStores = this._splitEventStore(interaction.mutatedEvents, mutatedKeysByDefId);
                const populate = (key) => {
                    if (!splitStates[key]) {
                        splitStates[key] = {
                ...
                for (const key in affectedStores) {
                    populate(key);
                }
                for (const key in mutatedStores) {
                    populate(key);
                }
            }
            return splitStates;
    ...
    function buildNavLinkAttrs(context, dateMarker, viewType = 'day', isTabbable = true) {
        ...
        const dateStr = dateEnv.format(dateMarker, viewType === 'week' ? WEEK_FORMAT : DAY_FORMAT);
        if (options.navLinks) {
            const zonedDate = dateEnv.toDate(dateMarker);
            const handleInteraction = (ev) => {
                const customAction = viewType === 'day' ? options.navLinkDayClick :
    ...
    function computeInnerRect(el, goWithinPadding = false, doFromWindowViewport) {
        const outerRect = doFromWindowViewport ? el.getBoundingClientRect() : computeRect(el);
    ...
    function computeClippedClientRect(el) {
        const clippingParents = getClippingParents(el);
        let rect = el.getBoundingClientRect();
        for (const clippingParent of clippingParents) {
            const intersection = intersectRects(rect, clippingParent.getBoundingClientRect());
            if (intersection) {
                rect = intersection;
            }
            else {
    ...
    */
    class PositionCache {
        constructor(originEl, els, isHorizontal, isVertical) {
            this.els = els;
            const originClientRect = this.originClientRect = originEl.getBoundingClientRect(); // relative to viewport top-left
            if (isHorizontal) {
    ...
    */
    ...
    class SegHierarchy {
        ...
        splitEntry(entry, barrier, hiddenEntries) {
            let partCnt = 0;
            const splitHiddenEntries = [];
            const entrySpan = entry.span;
            const barrierSpan = barrier.span;
        ...
        findInsertion(newEntry) {
            const { levelCoords, entriesByLevel, strictOrder, stackCnts } = this;
            ...
            for (let trackingLevel = 0; trackingLevel < levelCnt; trackingLevel += 1) {
                ...
                const trackingEntries = entriesByLevel[trackingLevel];
                let trackingEntry;
                const searchRes = binarySearch(trackingEntries, newEntry.span.start, getEntrySpanEnd); // find first entry after newEntry's end
                let lateralIndex = searchRes[0] + searchRes[1]; // if exact match (which doesn't collide), go to next one
                while ( // loop through entries that horizontally intersect
                (trackingEntry = trackingEntries[lateralIndex]) && // but not past the whole entry list
                    trackingEntry.span.start < newEntry.span.end // and not entirely past newEntry
                ) {
                    const trackingEntryBottom = trackingCoord + this.getEntryThickness(trackingEntry);
        ...
        toRects() {
            const { entriesByLevel, levelCoords } = this;
            const levelCnt = entriesByLevel.length;
            const rects = [];
            for (let level = 0; level < levelCnt; level += 1) {
                const entries = entriesByLevel[level];
                const levelCoord = levelCoords[level];
    ...
    function intersectSpans(span0, span1) {
        const start = Math.max(span0.start, span1.start);
        const end = Math.min(span0.end, span1.end);
        if (start < end) {
            return { start, end };
        }
        return null;
    ...
    function binarySearch(a, searchVal, getItemVal) {
        ...
        while (startIndex < endIndex) {
            ...
            const middleVal = getItemVal(a[middleIndex]);
    ...
    */
    ...
    function parseDragMeta(raw) {
        const { refined, extra } = refineProps(raw, DRAG_META_REFINERS);
        return {
    ...
    class TableDateCell extends BaseComponent {
        render() {
            const { dateEnv, options, theme, viewApi } = this.context;
            const { props } = this;
            const { date, dateProfile } = props;
            ...
            const navLinkAttrs = (!dayMeta.isDisabled && props.colCnt > 1)
                ? buildNavLinkAttrs(this.context, date)
                : {};
            const renderProps = Object.assign(Object.assign(Object.assign({ date: dateEnv.toDate(date), view: viewApi }, props.extraRenderProps), { text }), dayMeta);
    ...
    class TableDowCell extends BaseComponent {
        render() {
            const { props } = this;
            const { dateEnv, theme, viewApi, options } = this.context;
            ...
            const text = dateEnv.format(date, props.dayHeaderFormat);
            const renderProps = Object.assign(Object.assign(Object.assign(Object.assign({ // TODO: make this public?
    ...
    class NowTimer extends x$1 {
        ...
        computeTiming() {
            const { props, context } = this;
            const unroundedNow = addMs(this.initialNowDate, new Date().valueOf() - this.initialNowQueriedMs);
            const currentUnitStart = context.dateEnv.startOf(unroundedNow, props.unit);
            const nextUnitStart = context.dateEnv.add(currentUnitStart, createDuration(1, props.unit));
        ...
        setTimeout() {
            const { nextState, waitMs } = this.computeTiming();
    ...
    class DayHeader extends BaseComponent {
        ...
        render() {
            const { context } = this;
            const { dates, dateProfile, datesRepDistinctDays, renderIntro } = this.props;
            const dayHeaderFormat = this.createDayHeaderFormatter(context.options.dayHeaderFormat, datesRepDistinctDays, dates.length);
    ...
    class DayTableModel {
        ...
        sliceRange(range) {
            ...
            const segs = [];
            if (seriesSeg) {
                const { firstIndex, lastIndex } = seriesSeg;
    ...
    class Slicer {
        ...
        sliceProps(props, dateProfile, nextDayThreshold, context, ...extraArgs) {
            const { eventUiBases } = props;
            const eventSegs = this.sliceEventStore(props.eventStore, eventUiBases, dateProfile, nextDayThreshold, ...extraArgs);
            return {
        ...
        _sliceDateSpan(dateSpan, dateProfile, nextDayThreshold, eventUiBases, context, ...extraArgs) {
            ...
            const activeRange = computeActiveRange(dateProfile, Boolean(nextDayThreshold));
            ...
            if (activeDateSpanRange) {
                ...
                const segs = this.sliceRange(dateSpan.range, ...extraArgs);
        ...
        */
        sliceEventRange(eventRange, extraArgs) {
            ...
            const segs = this.sliceRange(dateRange, ...extraArgs);
            for (const seg of segs) {
                seg.eventRange = eventRange;
    ...
    */
    ...
    function isInteractionValid(interaction, dateProfile, context) {
        const { instances } = interaction.mutatedEvents;
        for (const instanceId in instances) {
    ...
    function isNewPropsValid(newProps, context) {
        ...
        const props = Object.assign({ businessHours: calendarState.businessHours, dateSelection: '', eventStore: calendarState.eventStore, eventUiBases: calendarState.eventUiBases, eventSelection: '', eventDrag: null, eventResize: null }, newProps);
    ...
    function isInteractionPropsValid(state, context, dateSpanMeta, filterConfig) {
        ...
        const otherEventStore = excludeInstances(state.eventStore, interaction.affectedEvents.instances);
        ...
        for (const subjectInstanceId in subjectInstances) {
            ...
            // overlap
            const { eventOverlap } = context.options;
            const eventOverlapFunc = typeof eventOverlap === 'function' ? eventOverlap : null;
            ...
            for (const subjectAllow of subjectConfig.allows) {
                const subjectDateSpan = Object.assign(Object.assign({}, dateSpanMeta), { range: subjectInstance.range, allDay: subjectDef.allDay });
    ...
    function isDateSelectionPropsValid(state, context, dateSpanMeta, filterConfig) {
        ...
        // overlap
        const { selectOverlap } = context.options;
        const selectOverlapFunc = typeof selectOverlap === 'function' ? selectOverlap : null;
        ...
        // allow (a function)
        for (const selectionAllow of selectionConfig.allows) {
            const fullDateSpan = Object.assign(Object.assign({}, dateSpanMeta), selection);
    ...
    class Scroller extends BaseComponent {
        ...
        render() {
            const { props } = this;
            const { liquid, liquidIsAbsolute } = props;
        ...
        needsXScrolling() {
            ...
            const { el } = this;
            const realClientWidth = this.el.getBoundingClientRect().width - this.getYScrollbarWidth();
            const { children } = el;
        ...
        needsYScrolling() {
            ...
            // inner-child margins and absolute positioning
            const { el } = this;
            const realClientHeight = this.el.getBoundingClientRect().height - this.getXScrollbarWidth();
            const { children } = el;
            for (let i = 0; i < children.length; i += 1) {
                const childEl = children[i];
    ...
    */
    ...
    function computeShrinkWidth(chunkEls) {
        ...
        let largestWidth = 0;
        for (const shrinkCell of shrinkCells) {
    ...
    function renderChunkContent(sectionConfig, chunkConfig, arg, isHeader) {
        const { expandRows } = arg;
        const content = typeof chunkConfig.content === 'function' ?
    ...
    class SimpleScrollGrid extends BaseComponent {
        ...
        renderChunkTd(sectionConfig, microColGroupNode, chunkConfig, isHeader) {
            ...
            const { props } = this;
            const { forceYScrollbars, scrollerClientWidths, scrollerClientHeights } = this.state;
            ...
            const overflowY = !props.liquid ? 'visible' :
        ...
        _handleScrollerEl(scrollerEl, key) {
            const section = getSectionByKey(this.props.sections, key);
            if (section) {
        ...
        computeScrollerDims() {
            ...
            const { scrollerRefs, scrollerElRefs } = this;
            let forceYScrollbars = false;
            const scrollerClientWidths = {};
            const scrollerClientHeights = {};
            for (const sectionKey in scrollerRefs.currentMap) {
                const scroller = scrollerRefs.currentMap[sectionKey];
            ...
            for (const section of this.props.sections) {
                ...
                const scrollerEl = scrollerElRefs.currentMap[sectionKey];
                if (scrollerEl) {
    ...
    class StandardEvent extends BaseComponent {
        render() {
            ...
            const { options } = context;
            const { seg } = props;
            const { ui } = seg.eventRange;
            const timeFormat = options.eventTimeFormat || props.defaultTimeFormat;
            const timeText = buildSegTimeText(seg, timeFormat, context, props.defaultDisplayEventTime, props.defaultDisplayEventEnd);
    ...
    function refineRenderProps$4(raw) {
        const { date, dateEnv, dateProfile, isMonthStart } = raw;
        const dayMeta = getDateMeta(date, raw.todayRange, null, dateProfile);
        const dayNumberText = raw.showDayNumber ? (dateEnv.format(date, isMonthStart ? raw.monthStartFormat : DAY_NUM_FORMAT)) : '';
    ...
    const WeekNumberContainer = (props) => (y(ViewContextType.Consumer, null, (context) => {
        ...
        const renderProps = { num, text, date };
    ...
    class Popover extends BaseComponent {
        ...
        render() {
            const { theme, options } = this.context;
            const { props, state } = this;
            const classNames = [
                'fc-popover',
        ...
        updateSize() {
            ...
            const { alignmentEl, alignGridTop } = this.props;
            const { rootEl } = this;
            const alignmentRect = computeClippedClientRect(alignmentEl);
            if (alignmentRect) {
                const popoverDims = rootEl.getBoundingClientRect();
                ...
                const origin = rootEl.offsetParent.getBoundingClientRect();
                applyStyle(rootEl, {
    ...
    class MorePopover extends DateComponent {
        ...
        render() {
            ...
            const { props } = this;
            const { startDate, todayRange, dateProfile } = props;
            const title = dateEnv.format(startDate, options.dayPopoverFormat);
    ...
    class MoreLinkContainer extends BaseComponent {
        constructor() {
            ...
            this.handleClick = (ev) => {
                ...
                const date = computeRange(props).start;
                function buildPublicSeg(seg) {
                    const { def, instance, range } = seg.eventRange;
                    return {
                        event: new EventImpl(context, def, instance),
        ...
        render() {
            ...
            return (y(ViewContextType.Consumer, null, (context) => {
                ...
                const { moreLinkText } = options;
                const { moreCnt } = props;
                const range = computeRange(props);
                const text = typeof moreLinkText === 'function' // TODO: eventually use formatWithOrdinals
                    ? moreLinkText.call(calendarApi, moreCnt)
                    : `+${moreCnt} ${moreLinkText}`;
                const hint = formatWithOrdinals(options.moreLinkHint, [moreCnt], text);
                const renderProps = {
                    num: moreCnt,
                    shortText: `+${moreCnt}`,
                    text,
    ...
    */
    ...
    function organizeRawLocales(explicitRawLocales) {
        ...
        for (const rawLocale of allRawLocales) {
    ...
    function parseLocale(codeArg, codes, raw) {
        const merged = mergeProps([MINIMAL_RAW_EN_LOCALE, raw], ['buttonText']);
    ...
    function buildViewDef(viewType, hash, defaultConfigs, overrideConfigs) {
        ...
        const queryProp = (name) => ((defaultConfig && defaultConfig[name] !== null) ? defaultConfig[name] :
    ...
    function parseViewConfig(input) {
        const rawOptions = typeof input === 'function' ?
            { component: input } :
            input;
    ...
    function buildViewSpec(viewDef, overrideConfigs, optionOverrides, dynamicOptionOverrides, localeDefaults) {
        ...
        const queryButtonText = (optionsSubset) => {
        ...
        const queryButtonTitle = (optionsSubset) => {
            const buttonHints = optionsSubset.buttonHints || {};
            const buttonKey = viewDef.defaults.buttonTextKey; // use same key as text
    ...
    function fetchSource$1(eventSource, fetchRange, isRefetch, context) {
        const { options, calendarApi } = context;
        const sourceDef = context.pluginHooks.eventSourceDefs[eventSource.sourceDefId];
        const fetchId = guid();
        sourceDef.fetch({
            eventSource,
            range: fetchRange,
    ...
    function parseToolbars(calendarOptions, calendarOptionOverrides, theme, viewSpecs, calendarApi) {
        const header = calendarOptions.headerToolbar ? parseToolbar(calendarOptions.headerToolbar, calendarOptions, calendarOptionOverrides, theme, viewSpecs, calendarApi) : null;
        const footer = calendarOptions.footerToolbar ? parseToolbar(calendarOptions.footerToolbar, calendarOptions, calendarOptionOverrides, theme, viewSpecs, calendarApi) : null;
    ...
    function parseToolbar(sectionStrHash, calendarOptions, calendarOptionOverrides, theme, viewSpecs, calendarApi) {
        ...
        let hasTitle = false;
        for (const sectionName in sectionStrHash) {
            const sectionStr = sectionStrHash[sectionName];
            const sectionRes = parseSection(sectionStr, calendarOptions, calendarOptionOverrides, theme, viewSpecs, calendarApi);
    ...
    */
    ...
    theme, viewSpecs, calendarApi) {
        ...
        const calendarButtonHintOverrides = calendarOptionOverrides.buttonHints || {};
        const calendarButtonHints = calendarOptions.buttonHints || {};
        const sectionSubstrs = sectionStr ? sectionStr.split(' ') : [];
        const viewsWithButtons = [];
        let hasTitle = false;
        const widgets = sectionSubstrs.map((buttonGroupStr) => (buttonGroupStr.split(',').map((buttonName) => {
            if (buttonName === 'title') {
                hasTitle = true;
                return { buttonName };
            }
            ...
            if ((customButtonProps = calendarCustomButtons[buttonName])) {
                ...
                const textFallback = viewSpec.buttonTextOverride ||
                ...
                if (buttonName === 'prevYear' || buttonName === 'nextYear') {
                    const prevOrNext = buttonName === 'prevYear' ? 'prev' : 'next';
    ...
    */
    function handleEventSources(inputs, context) {
        const unfoundSources = hashValuesToArray(context.getCurrentData().eventSources);
        ...
        for (const unfoundSource of unfoundSources) {
            context.dispatch({
                type: 'REMOVE_EVENT_SOURCE',
    ...
    */
    ...
    class CalendarDataManager {
        constructor(props) {
            ...
            const optionsData = this.computeOptionsData(props.optionOverrides, dynamicOptionOverrides, props.calendarApi);
            const currentViewType = optionsData.calendarOptions.initialView || optionsData.pluginHooks.initialView;
            const currentViewData = this.computeCurrentViewData(currentViewType, optionsData, props.optionOverrides, dynamicOptionOverrides);
            // wire things up
            // TODO: not DRY
            ...
            for (const callback of optionsData.pluginHooks.contextInit) {
                callback(calendarContext);
            }
            // NOT DRY
            const eventSources = initEventSources(optionsData.calendarOptions, dateProfile, calendarContext);
            const initialState = {
            ...
            const contextAndState = Object.assign(Object.assign({}, calendarContext), initialState);
            for (const reducer of optionsData.pluginHooks.reducers) {
        ...
        _handleAction(action) {
            const { props, state, emitter } = this;
            const dynamicOptionOverrides = reduceDynamicOptionOverrides(state.dynamicOptionOverrides, action);
            const optionsData = this.computeOptionsData(props.optionOverrides, dynamicOptionOverrides, props.calendarApi);
            ...
            const eventSources = reduceEventSources(state.eventSources, action, dateProfile, calendarContext);
            const eventStore = reduceEventStore(state.eventStore, action, eventSources, dateProfile, calendarContext);
            const isEventsLoading = computeEventSourcesLoading(eventSources); // BAD. also called in this func in computeIsLoading
            const renderableEventStore = (isEventsLoading && !currentViewData.options.progressiveEventRendering) ?
                (state.renderableEventStore || eventStore) : // try from previous state
                eventStore;
            const { eventUiSingleBase, selectionConfig } = this.buildViewUiProps(calendarContext); // will memoize obj
            const eventUiBySource = this.buildEventUiBySource(eventSources);
            const eventUiBases = this.buildEventUiBases(renderableEventStore.defs, eventUiSingleBase, eventUiBySource);
            const newState = {
                dynamicOptionOverrides,
                currentViewType,
                currentDate,
            ...
            const contextAndState = Object.assign(Object.assign({}, calendarContext), newState);
            for (const reducer of optionsData.pluginHooks.reducers) {
                Object.assign(newState, reducer(state, action, contextAndState)); // give the OLD state, for old value
            }
            const wasLoading = computeIsLoading(state, calendarContext);
            const isLoading = computeIsLoading(newState, calendarContext);
        ...
        updateData() {
            ...
            const optionsData = this.computeOptionsData(props.optionOverrides, state.dynamicOptionOverrides, props.calendarApi);
            const currentViewData = this.computeCurrentViewData(state.currentViewType, optionsData, props.optionOverrides, state.dynamicOptionOverrides);
            const data = this.data = Object.assign(Object.assign(Object.assign({ viewTitle: this.buildTitle(state.dateProfile, currentViewData.options, optionsData.dateEnv), calendarApi: props.calendarApi, dispatch: this.dispatch, emitter: this.emitter, getCurrentData: this.getCurrentData }, optionsData), currentViewData), state);
        ...
        computeOptionsData(optionOverrides, dynamicOptionOverrides, calendarApi) {
            ...
            const { refinedOptions, pluginHooks, localeDefaults, availableLocaleData, extra, } = this.processRawCalendarOptions(optionOverrides, dynamicOptionOverrides);
            warnUnknownOptions(extra);
            const dateEnv = this.buildDateEnv(refinedOptions.timeZone, refinedOptions.locale, refinedOptions.weekNumberCalculation, refinedOptions.firstDay, refinedOptions.weekText, pluginHooks, availableLocaleData, refinedOptions.defaultRangeSeparator);
            const viewSpecs = this.buildViewSpecs(pluginHooks.views, this.stableOptionOverrides, this.stableDynamicOptionOverrides, localeDefaults);
            const theme = this.buildTheme(refinedOptions, pluginHooks);
            const toolbarConfig = this.parseToolbars(refinedOptions, this.stableOptionOverrides, theme, viewSpecs, calendarApi);
            return this.stableCalendarOptionsData = {
        ...
        processRawCalendarOptions(optionOverrides, dynamicOptionOverrides) {
            const { locales, locale } = mergeRawOptions([
            ...
            const localeDefaults = this.buildLocale(locale || availableLocaleData.defaultCode, availableRawLocales).options;
            const pluginHooks = this.buildPluginHooks(optionOverrides.plugins || [], globalPlugins);
            const refiners = this.currentCalendarOptionsRefiners = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, BASE_OPTION_REFINERS), CALENDAR_LISTENER_REFINERS), CALENDAR_OPTION_REFINERS), pluginHooks.listenerRefiners), pluginHooks.optionRefiners);
            const extra = {};
        ...
        _computeCurrentViewData(viewType, optionsData, optionOverrides, dynamicOptionOverrides) {
            ...
            const { refinedOptions, extra } = this.processRawViewOptions(viewSpec, optionsData.pluginHooks, optionsData.localeDefaults, optionOverrides, dynamicOptionOverrides);
            warnUnknownOptions(extra);
            ...
            const viewApi = this.buildViewApi(viewType, this.getCurrentData, optionsData.dateEnv);
        ...
        processRawViewOptions(viewSpec, pluginHooks, localeDefaults, optionOverrides, dynamicOptionOverrides) {
            ...
            const refiners = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, BASE_OPTION_REFINERS), CALENDAR_LISTENER_REFINERS), CALENDAR_OPTION_REFINERS), VIEW_OPTION_REFINERS), pluginHooks.listenerRefiners), pluginHooks.optionRefiners);
            const refined = {};
    ...
    function buildDateEnv$1(timeZone, explicitLocale, weekNumberCalculation, firstDay, weekText, pluginHooks, availableLocaleData, defaultSeparator) {
        const locale = buildLocale(explicitLocale || availableLocaleData.defaultCode, availableLocaleData.map);
        return new DateEnv({
    ...
    function buildTheme(options, pluginHooks) {
        const ThemeClass = pluginHooks.themeClasses[options.themeSystem] || StandardTheme;
        return new ThemeClass(options);
    }
    function buildDateProfileGenerator(props) {
        const DateProfileGeneratorClass = props.dateProfileGeneratorClass || DateProfileGenerator;
    ...
    function computeIsLoading(state, context) {
        for (const isLoadingFunc of context.pluginHooks.isLoadingFuncs) {
    ...
    class ToolbarSection extends BaseComponent {
        render() {
            const children = this.props.widgetGroups.map((widgetGroup) => this.renderWidgetGroup(widgetGroup));
        ...
        renderWidgetGroup(widgetGroup) {
            ...
            for (const widget of widgetGroup) {
                const { buttonName, buttonClick, buttonText, buttonIcon, buttonHint } = widget;
                if (buttonName === 'title') {
                    isOnlyButtons = false;
                    children.push(y("h2", { className: "fc-toolbar-title", id: props.titleId }, props.title));
                }
                else {
                    const isPressed = buttonName === props.activeButton;
                    const isDisabled = (!props.isTodayEnabled && buttonName === 'today') ||
                        (!props.isPrevEnabled && buttonName === 'prev') ||
                        (!props.isNextEnabled && buttonName === 'next');
                    const buttonClasses = [`fc-${buttonName}-button`, theme.getClass('button')];
                    if (isPressed) {
            ...
            if (children.length > 1) {
                const groupClassName = (isOnlyButtons && theme.getClass('buttonGroup')) || '';
    ...
    class Toolbar extends BaseComponent {
        render() {
            const { model, extraClassName } = this.props;
            let forceLtr = false;
            let startContent;
            let endContent;
            const sectionWidgets = model.sectionWidgets;
            const centerContent = sectionWidgets.center;
        ...
        constructor(settings) {
            ...
            this.handleSegClick = (ev, segEl) => {
                ...
                if (seg && // might be the <div> surrounding the more link
                    ...
                    const hasUrlContainer = elementClosest(ev.target, '.fc-event-forced-url');
    ...
    */
    ...
    class CalendarContent extends PureComponent {
        constructor() {
            ...
            this.registerInteractiveComponent = (component, settingsInput) => {
                ...
                const interactionClasses = DEFAULT_INTERACTIONS.concat(this.props.pluginHooks.componentInteractions);
                const interactions = interactionClasses.map((TheInteractionClass) => new TheInteractionClass(settings));
                this.interactionsStore[component.uid] = interactions;
                interactionSettingsStore[component.uid] = settings;
            };
            this.unregisterInteractiveComponent = (component) => {
                const listeners = this.interactionsStore[component.uid];
                if (listeners) {
        ...
        */
        render() {
            const { props } = this;
            const { toolbarConfig, options } = props;
            const toolbarProps = this.buildToolbarProps(props.viewSpec, props.dateProfile, props.dateProfileGenerator, props.currentDate, getNow(props.options.now, props.dateEnv), // TODO: use NowTimer????
            props.viewTitle);
            let viewVGrow = false;
            let viewHeight = '';
            let viewAspectRatio;
            ...
            const viewContext = this.buildViewContext(props.viewSpec, props.viewApi, props.options, props.dateProfileGenerator, props.dateEnv, props.theme, props.pluginHooks, props.dispatch, props.getCurrentData, props.emitter, props.calendarApi, this.registerInteractiveComponent, this.unregisterInteractiveComponent);
            const viewLabelId = (toolbarConfig.header && toolbarConfig.header.hasTitle)
        ...
        componentDidMount() {
            ...
            const { propSetHandlers } = props.pluginHooks;
        ...
        componentWillUnmount() {
            ...
            for (const interaction of this.calendarInteractions) {
                interaction.destroy();
            }
        ...
        buildAppendContent() {
            const { props } = this;
            const children = props.pluginHooks.viewContainerAppends.map((buildAppendContent) => buildAppendContent(props));
            return y(_, {}, ...children);
        }
        renderView(props) {
            const { pluginHooks } = props;
            const { viewSpec } = props;
            const viewProps = {
                dateProfile: props.dateProfile,
            ...
            const transformers = this.buildViewPropTransformers(pluginHooks.viewPropsTransformers);
            for (const transformer of transformers) {
    ...
    class Calendar extends CalendarImpl {
        ...
        setClassNames(classNames) {
            if (!isArraysEqual(classNames, this.currentClassNames)) {
                ...
                for (const className of this.currentClassNames) {
    ...
    function formatRange(startInput, endInput, options) {
        const dateEnv = buildDateEnv(typeof options === 'object' && options ? options : {}); // pass in if non-null object
        const formatter = createFormatter(options);
    ...
    function buildDateEnv(settings) {
        const locale = buildLocale(settings.locale || 'en', organizeRawLocales([]).map); // TODO: don't hardcode 'en' everywhere
        ...
        constructor(containerEl) {
            ...
            this.handleMouseDown = (ev) => {
                if (!this.shouldIgnoreMouse() &&
                    ...
                    this.tryStart(ev)) {
                    const pev = this.createEventFromMouse(ev, true);
            ...
            this.handleMouseMove = (ev) => {
                const pev = this.createEventFromMouse(ev);
            ...
            this.handleTouchStart = (ev) => {
                if (this.tryStart(ev)) {
                    ...
                    const pev = this.createEventFromTouch(ev, true);
                    this.emitter.trigger('pointerdown', pev);
                    this.initScrollWatch(pev);
            ...
            this.handleTouchMove = (ev) => {
                const pev = this.createEventFromTouch(ev);
                this.recordCoords(pev);
                this.emitter.trigger('pointermove', pev);
            };
            ...
            this.handleScroll = (ev) => {
                if (!this.shouldIgnoreMove) {
                    const pageX = (window.pageXOffset - this.prevScrollX) + this.prevPageX;
                    const pageY = (window.pageYOffset - this.prevScrollY) + this.prevPageY;
    ...
    class AutoScroller {
        constructor() {
            ...
            this.animate = () => {
                if (this.isAnimating) { // wasn't cancelled between animation calls
                    const edge = this.computeBestEdge(this.pointerScreenX + window.pageXOffset, this.pointerScreenY + window.pageYOffset);
                    if (edge) {
                        const now = getTime();
        ...
        handleMove(pageX, pageY) {
            if (this.isEnabled) {
                ...
                const yDelta = this.pointerScreenY === null ? 0 : pointerScreenY - this.pointerScreenY;
                const xDelta = this.pointerScreenX === null ? 0 : pointerScreenX - this.pointerScreenX;
                if (yDelta < 0) {
        ...
        stop() {
            if (this.isEnabled) {
                this.isAnimating = false; // will stop animation
                for (const scrollCache of this.scrollCaches) {
                    scrollCache.destroy();
                }
                this.scrollCaches = null;
            }
        ...
        handleSide(edge, seconds) {
            const { scrollCache } = edge;
            const { edgeThreshold } = this;
        ...
        computeBestEdge(left, top) {
            const { edgeThreshold } = this;
            let bestSide = null;
            const scrollCaches = this.scrollCaches || [];
            for (const scrollCache of scrollCaches) {
        ...
        queryScrollEls(scrollStartEl) {
            const els = [];
            for (const query of this.scrollQuery) {
    ...
    */
    class FeaturefulElementDragging extends ElementDragging {
        constructor(containerEl, selector) {
            ...
            const pointer = this.pointer = new PointerDragging(containerEl);
    ...
    */
    class HitDragging {
        ...
        processFirstCoord(ev) {
            ...
            const initialHit = this.initialHit = this.queryHitForOffset(adjustedPoint.left, adjustedPoint.top);
            if (initialHit) {
        ...
        handleMove(ev, forceHandle) {
            const hit = this.queryHitForOffset(ev.pageX + this.coordAdjust.left, ev.pageY + this.coordAdjust.top);
        ...
        queryHitForOffset(offsetLeft, offsetTop) {
            const { droppableStore, offsetTrackers } = this;
            let bestHit = null;
            for (const id in droppableStore) {
                ...
                if (offsetTracker && // wasn't destroyed mid-drag
                    ...
                    positionLeft >= 0 && positionLeft < width &&
                        ...
                        const hit = component.queryHit(positionLeft, positionTop, width, height);
                        if (hit && (
    ...
    function buildDatePointApiWithContext(dateSpan, context) {
        const props = {};
        for (const transform of context.pluginHooks.datePointTransforms) {
            Object.assign(props, transform(dateSpan, context));
        }
        ...
        constructor(settings) {
            ...
            this.handleDragEnd = (ev) => {
                ...
                if (!pointer.wasTouchScroll) {
                    const { initialHit, finalHit } = this.hitDragging;
                    if (initialHit && finalHit && isHitsEqual(initialHit, finalHit)) {
                        const { context } = component;
                        const arg = Object.assign(Object.assign({}, buildDatePointApiWithContext(initialHit.dateSpan, context)), { dayEl: initialHit.dayEl, jsEvent: ev.origEvent, view: context.viewApi || context.calendarApi.view });
            ...
            const hitDragging = this.hitDragging = new HitDragging(this.dragging, interactionSettingsToStore(settings));
    ...
    */
    class DateSelecting extends Interaction {
        constructor(settings) {
            ...
            this.handlePointerDown = (ev) => {
                const { component, dragging } = this;
                const { options } = component.context;
                const canSelect = options.selectable &&
            ...
            this.handleHitUpdate = (hit, isFinal) => {
                ...
                let isInvalid = false;
                if (hit) {
                    const initialHit = this.hitDragging.initialHit;
                    const disallowed = hit.componentId === initialHit.componentId
            ...
            const { options } = component.context;
            const dragging = this.dragging = new FeaturefulElementDragging(settings.el);
    ...
    function joinHitsIntoSelection(hit0, hit1, dateSelectionTransformers) {
        ...
        const props = {};
        for (const transformer of dateSelectionTransformers) {
    ...
    class EventDragging extends Interaction {
        constructor(settings) {
            ...
            this.handlePointerDown = (ev) => {
                ...
                const subjectSeg = this.subjectSeg = getElSeg(ev.subjectEl);
                const eventRange = this.eventRange = subjectSeg.eventRange;
                ...
                const isValid = component.isValidSegDownEl(origTarget) &&
            ...
            this.handleDragStart = (ev) => {
                ...
                const eventRange = this.eventRange;
                const eventInstanceId = eventRange.instance.instanceId;
                if (ev.isTouch) {
            ...
            this.handleHitUpdate = (hit, isFinal) => {
                ...
                const relevantEvents = this.relevantEvents;
                const initialHit = this.hitDragging.initialHit;
                const initialContext = this.component.context;
            ...
            this.handleDragEnd = (ev) => {
                if (this.isDragging) {
                    const initialContext = this.component.context;
                    const initialView = initialContext.viewApi;
                    const { receivingContext, validMutation } = this;
                    const eventDef = this.eventRange.def;
                    const eventInstance = this.eventRange.instance;
                    const eventApi = new EventImpl(initialContext, eventDef, eventInstance);
                    const relevantEvents = this.relevantEvents;
                    const mutatedRelevantEvents = this.mutatedRelevantEvents;
                    const { finalHit } = this.hitDragging;
                    ...
                    if (validMutation) {
                        ...
                        if (receivingContext === initialContext) {
                            const updatedEventApi = new EventImpl(initialContext, mutatedRelevantEvents.defs[eventDef.defId], eventInstance ? mutatedRelevantEvents.instances[eventInstance.instanceId] : null);
                            ...
                            const transformed = {};
                            for (const transformer of initialContext.getCurrentData().pluginHooks.eventDropTransformers) {
                            ...
                            const addedEventDef = mutatedRelevantEvents.defs[eventDef.defId];
                            const addedEventInstance = mutatedRelevantEvents.instances[eventInstance.instanceId];
                            const addedEventApi = new EventImpl(receivingContext, addedEventDef, addedEventInstance);
            ...
            dragging.touchScrollAllowed = false;
            dragging.autoScroller.isEnabled = options.dragScroll;
            const hitDragging = this.hitDragging = new HitDragging(this.dragging, interactionSettingsStore);
    ...
    function computeEventMutation(hit0, hit1, massagers) {
        ...
        const delta = diffDates(date0, date1, hit0.context.dateEnv, hit0.componentId === hit1.componentId ?
            hit0.largeUnit :
            null);
        ...
        for (const massager of massagers) {
    ...
    class EventResizing extends Interaction {
        constructor(settings) {
            ...
            this.handlePointerDown = (ev) => {
                const { component } = this;
                const segEl = this.querySegEl(ev);
                const seg = getElSeg(segEl);
                const eventRange = this.eventRange = seg.eventRange;
            ...
            this.handleDragEnd = (ev) => {
                ...
                const eventDef = this.eventRange.def;
                const eventInstance = this.eventRange.instance;
                const eventApi = new EventImpl(context, eventDef, eventInstance);
                const relevantEvents = this.relevantEvents;
                const mutatedRelevantEvents = this.mutatedRelevantEvents;
                ...
                if (this.validMutation) {
                    const updatedEventApi = new EventImpl(context, mutatedRelevantEvents.defs[eventDef.defId], eventInstance ? mutatedRelevantEvents.instances[eventInstance.instanceId] : null);
                    context.dispatch({
                        type: 'MERGE_EVENTS',
                        eventStore: mutatedRelevantEvents,
                    });
    ...
    function computeMutation(hit0, hit1, isFromStart, instanceRange) {
        const dateEnv = hit0.context.dateEnv;
        const date0 = hit0.dateSpan.range.start;
        const date1 = hit1.dateSpan.range.start;
        const delta = diffDates(date0, date1, dateEnv, hit0.largeUnit);
        if (isFromStart) {
    ...
    class UnselectAuto {
        constructor(context) {
            ...
            this.onDocumentPointerDown = (pev) => {
                const unselectCancel = this.context.options.unselectCancel;
                const downEl = getEventTargetViaRoot(pev.origEvent);
                this.matchesCancel = !!elementClosest(downEl, unselectCancel);
                this.matchesEvent = !!elementClosest(downEl, EventDragging.SELECTOR); // interaction started on an event?
            };
            this.onDocumentPointerUp = (pev) => {
                const { context } = this;
                const { documentPointer } = this;
                const calendarState = context.getCurrentData();
            ...
            const documentPointer = this.documentPointer = new PointerDragging(document);
    ...
    */
    class ExternalElementDragging {
        constructor(dragging, suppliedDragMeta) {
            ...
            this.handleHitUpdate = (hit, isFinal, ev) => {
                const { dragging } = this.hitDragging;
                let receivingContext = null;
            ...
            this.handleDragEnd = (pev) => {
                const { receivingContext, droppableEvent } = this;
                this.clearDrag();
                if (receivingContext && droppableEvent) {
                    const finalHit = this.hitDragging.finalHit;
                    const finalView = finalHit.context.viewApi;
                    const dragMeta = this.dragMeta;
                    receivingContext.emitter.trigger('drop', Object.assign(Object.assign({}, buildDatePointApiWithContext(finalHit.dateSpan, receivingContext)), { draggedEl: pev.subjectEl, jsEvent: pev.origEvent, view: finalView }));
                    if (dragMeta.create) {
                        const addingEvents = eventTupleToStore(droppableEvent);
                        receivingContext.dispatch({
                            type: 'MERGE_EVENTS',
            ...
            const hitDragging = this.hitDragging = new HitDragging(dragging, interactionSettingsStore);
    ...
    function computeEventForDateSpan(dateSpan, dragMeta, context) {
        const defProps = Object.assign({}, dragMeta.leftoverProps);
        for (const transform of context.pluginHooks.externalDefTransforms) {
            Object.assign(defProps, transform(dateSpan, dragMeta));
        }
        const { refined, extra } = refineEventDef(defProps, context);
        const def = parseEventDef(refined, extra, dragMeta.sourceId, dateSpan.allDay, context.options.forceEventDuration || Boolean(dragMeta.duration), // hasEnd
        context);
    ...
    */
    class ExternalDraggable {
        constructor(el, settings = {}) {
            this.handlePointerDown = (ev) => {
                const { dragging } = this;
                const { minDistance, longPressDelay } = this.settings;
    ...
    */
    ...
    class TableView extends DateComponent {
        ...
        renderHScrollLayout(headerRowContent, bodyContent, colCnt, dayMinWidth) {
            ...
            const { props, context } = this;
            const stickyHeaderDates = !props.forPrint && getStickyHeaderDates(context.options);
            const stickyFooterScrollbar = !props.forPrint && getStickyFooterScrollbar(context.options);
            const sections = [];
            if (headerRowContent) {
                sections.push({
                    type: 'header',
    ...
    function hasListItemDisplay(seg) {
        const { display } = seg.eventRange.ui;
    ...
    class TableListItemEvent extends BaseComponent {
        render() {
            ...
            const { options } = context;
            const { seg } = props;
            const timeFormat = options.eventTimeFormat || DEFAULT_TABLE_EVENT_TIME_FORMAT;
            const timeText = buildSegTimeText(seg, timeFormat, context, true, props.defaultDisplayEventEnd);
    ...
    class TableCellMoreLink extends BaseComponent {
        ...
        render() {
            const { props } = this;
            const { allSegs, invisibleSegs } = this.compileSegs(props.singlePlacements);
            return (y(MoreLinkContainer, { elClasses: ['fc-daygrid-more-link'], dateProfile: props.dateProfile, todayRange: props.todayRange, allDayDate: props.allDayDate, moreCnt: props.moreCnt, allSegs: allSegs, hiddenSegs: invisibleSegs, alignmentElRef: props.alignmentElRef, alignGridTop: props.alignGridTop, extraDateSpan: props.extraDateSpan, popoverContent: () => {
                    const isForcedInvisible = (props.eventDrag ? props.eventDrag.affectedInstances : null) ||
    ...
    class TableCell extends DateComponent {
        ...
        render() {
            const { context, props, state, rootElRef } = this;
    ...
    dayMaxEvents, dayMaxEventRows, strictOrder, segHeights, maxContentHeight, cells) {
        const hierarchy = new DayGridSegHierarchy((segEntry) => {
            // TODO: more DRY with generateSegUid
            const segUid = segs[segEntry.index].eventRange.instance.instanceId +
                ':' + segEntry.span.start +
                ':' + (segEntry.span.end - 1);
        ...
        const segInputs = [];
        const unknownHeightSegs = [];
        for (let i = 0; i < segs.length; i += 1) {
            const seg = segs[i];
            const segUid = generateSegUid(seg);
            const eventHeight = segHeights[segUid];
            if (eventHeight != null) {
                segInputs.push({
                    index: i,
                    span: {
        ...
        const { singleColPlacements, multiColPlacements, leftoverMargins } = placeRects(segRects, segs, cells);
        const moreCnts = [];
        const moreMarginTops = [];
        // add segs with unknown heights
        for (const seg of unknownHeightSegs) {
        ...
        for (const hiddenEntry of hiddenEntries) {
            const seg = segs[hiddenEntry.index];
            const hiddenSpan = hiddenEntry.span;
    ...
    function placeRects(allRects, segs, cells) {
        const rectsByEachCol = groupRectsByEachCol(allRects, cells.length);
        const singleColPlacements = [];
        const multiColPlacements = [];
        const leftoverMargins = [];
        for (let col = 0; col < cells.length; col += 1) {
            const rects = rectsByEachCol[col];
            ...
            for (const rect of rects) {
                const seg = segs[rect.index];
                const isAbsolute = rect.span.end - rect.span.start > 1; // multi-column?
                const isFirstCol = rect.span.start === col;
    ...
    function resliceSeg(seg, spanStart, spanEnd, cells) {
        ...
        const slicedRange = intersectRanges(origRange, {
    ...
    class TableRow extends DateComponent {
        ...
        render() {
            ...
            const { singleColPlacements, multiColPlacements, moreCnts, moreMarginTops } = computeFgSegPlacement(sortEventSegs(props.fgEventSegs, options.eventOrder), props.dayMaxEvents, props.dayMaxEventRows, options.eventOrderStrict, state.segHeights, state.maxContentHeight, props.cells);
            ...
            return (y("tr", { ref: this.rootElRef, role: "row" },
                ...
                props.cells.map((cell, col) => {
                    const normalFgNodes = this.renderFgSegs(col, props.forPrint ? singleColPlacements[col] : multiColPlacements[col], props.todayRange, isForcedInvisible);
                    const mirrorFgNodes = this.renderFgSegs(col, buildMirrorPlacements$1(mirrorSegsByCol[col], multiColPlacements), props.todayRange, {}, Boolean(props.eventDrag), Boolean(props.eventResize), false);
        ...
        renderFgSegs(col, segPlacements, todayRange, isForcedInvisible, isDragging, isResizing, isDateSelecting) {
            const { context } = this;
            const { eventSelection } = this.props;
            const { framePositions } = this.state;
            ...
            if (framePositions) {
                for (const placement of segPlacements) {
                    ...
                    const isVisible = placement.isVisible && !isForcedInvisible[instanceId];
                    const isAbsolute = placement.isAbsolute;
                    let left = '';
                    let right = '';
                    if (isAbsolute) {
                        if (context.isRtl) {
        ...
        updateSizing(isExternalSizingChange) {
            const { props, state, frameElRefs } = this;
            if (!props.forPrint &&
                props.clientWidth !== null // positioning ready?
            ) {
                if (isExternalSizingChange) {
                    const frameEls = props.cells.map((cell) => frameElRefs.currentMap[cell.key]);
                    if (frameEls.length) {
                        const originEl = this.rootElRef.current;
                        const newPositionCache = new PositionCache(originEl, frameEls, true, // isHorizontal
                        false);
        ...
        querySegHeights() {
            const segElMap = this.segHarnessRefs.currentMap;
            const segHeights = {};
            // get the max height amongst instance segs
            for (const segUid in segElMap) {
                const height = Math.round(segElMap[segUid].getBoundingClientRect().height);
    ...
    function buildMirrorPlacements$1(mirrorSegs, colPlacements) {
        ...
        const topsByInstanceId = buildAbsoluteTopHash$1(colPlacements); // TODO: cache this at first render?
    ...
    function buildAbsoluteTopHash$1(colPlacements) {
        const topsByInstanceId = {};
        for (const placements of colPlacements) {
    ...
    class TableRows extends DateComponent {
        ...
        render() {
            ...
            const businessHourSegsByRow = this.splitBusinessHourSegs(props.businessHourSegs, rowCnt);
            ...
            const dateSelectionSegsByRow = this.splitDateSelectionSegs(props.dateSelectionSegs, rowCnt);
            ...
            const cellMinHeight = (rowCnt >= 7 && props.clientWidth) ?
        ...
        queryHit(positionLeft, positionTop) {
            const { colPositions, rowPositions } = this;
            const col = colPositions.leftToIndex(positionLeft);
            const row = rowPositions.topToIndex(positionTop);
    ...
    class DayTableView extends TableView {
        ...
        render() {
            const { options, dateProfileGenerator } = this.context;
            const { props } = this;
            const dayTableModel = this.buildDayTableModel(props.dateProfile, dateProfileGenerator);
            const headerContent = options.dayHeaders && (y(DayHeader, { ref: this.headerRef, dateProfile: props.dateProfile, dates: dayTableModel.headerDates, datesRepDistinctDays: dayTableModel.rowCnt === 1 }));
            const bodyContent = (contentArg) => (y(DayTable, { ref: this.tableRef, dateProfile: props.dateProfile, dayTableModel: dayTableModel, businessHours: props.businessHours, dateSelection: props.dateSelection, eventStore: props.eventStore, eventUiBases: props.eventUiBases, eventSelection: props.eventSelection, eventDrag: props.eventDrag, eventResize: props.eventResize, nextDayThreshold: options.nextDayThreshold, colGroupNode: contentArg.tableColGroupNode, tableMinWidth: contentArg.tableMinWidth, dayMaxEvents: options.dayMaxEvents, dayMaxEventRows: options.dayMaxEventRows, showWeekNumbers: options.weekNumbers, expandRows: !props.isHeightAuto, headerAlignElRef: this.headerElRef, clientWidth: contentArg.clientWidth, clientHeight: contentArg.clientHeight, forPrint: props.forPrint }));
    ...
    class TableDateProfileGenerator extends DateProfileGenerator {
        ...
        buildRenderRange(currentRange, currentRangeUnit, isRangeAllDay) {
            const renderRange = super.buildRenderRange(currentRange, currentRangeUnit, isRangeAllDay);
            const { props } = this;
    ...
    function buildDayTableRenderRange(props) {
        const { dateEnv, currentRange } = props;
        ...
        if (props.fixedWeekCount) {
            ...
            const lastMonthRenderStart = dateEnv.startOfWeek(dateEnv.startOfMonth(addDays(currentRange.end, -1)));
            const rowCnt = Math.ceil(// could be partial weeks due to hiddenDays
    ...
    */
    ...
    function buildPositioning(segInputs, strictOrder, maxStackCnt) {
        ...
        const hiddenEntries = hierarchy.addSegs(segInputs);
        const hiddenGroups = groupIntersectingEntries(hiddenEntries);
    ...
    function findNextLevelSegs(hierarchy, subjectLevel, subjectLateral) {
        ...
        for (; level < levelCnt; level += 1) {
            const entries = entriesByLevel[level];
            let entry;
            const searchIndex = binarySearch(entries, subjectEntry.span.start, getEntrySpanEnd);
    ...
    function webToRects(topLevelNodes) {
        ...
        const processNode = cacheable((node, levelCoord, stackDepth) => buildEntryKey(node), (node, levelCoord, stackDepth) => {
            const rect = Object.assign(Object.assign({}, node), { levelCoord,
    ...
    class TimeCol extends BaseComponent {
        ...
        segIsInvisible, isDragging, isResizing, isDateSelecting, forcedKey) {
            ...
            const { segPlacements, hiddenGroups } = computeFgSegPlacements$1(segs, segVCoords, eventOrderStrict, eventMaxStack);
            return (y(_, null,
    ...
    function buildSlatMetas(slotMinTime, slotMaxTime, explicitLabelInterval, slotDuration, dateEnv) {
        ...
        while (asRoughMs(slatTime) < asRoughMs(slotMaxTime)) {
            ...
            const isLabeled = wholeDivideDurations(slatIterator, labelInterval) !== null;
            metas.push({
                date,
                time: slatTime,
                key: date.toISOString(),
    ...
    class DayTimeColsView extends TimeColsView {
        ...
        render() {
            ...
            const headerContent = options.dayHeaders && (y(DayHeader, { dates: dayTableModel.headerDates, dateProfile: dateProfile, datesRepDistinctDays: true, renderIntro: hasAttachedAxis ? this.renderHeadAxis : null }));
            const allDayContent = (options.allDaySlot !== false) && ((contentArg) => (y(DayTable, Object.assign({}, splitProps.allDay, { dateProfile: dateProfile, dayTableModel: dayTableModel, nextDayThreshold: options.nextDayThreshold, tableMinWidth: contentArg.tableMinWidth, colGroupNode: contentArg.tableColGroupNode, renderRowIntro: hasAttachedAxis ? this.renderTableRowAxis : null, showWeekNumbers: false, expandRows: false, headerAlignElRef: this.headerElRef, clientWidth: contentArg.clientWidth, clientHeight: contentArg.clientHeight, forPrint: props.forPrint }, this.getAllDayMaxEventProps()))));
            const timeGridContent = (contentArg) => (y(DayTimeCols, Object.assign({}, splitProps.timed, { dayTableModel: dayTableModel, dateProfile: dateProfile, axis: hasAttachedAxis, slotDuration: options.slotDuration, slatMetas: slatMetas, forPrint: props.forPrint, tableColGroupNode: contentArg.tableColGroupNode, tableMinWidth: contentArg.tableMinWidth, clientWidth: contentArg.clientWidth, clientHeight: contentArg.clientHeight, onSlatCoords: this.handleSlatCoords, expandRows: contentArg.expandRows, onScrollTopRequest: this.handleScrollTopRequest })));
    ...
    class ListViewHeaderRow extends BaseComponent {
        ...
        render() {
            const { theme, dateEnv, options, viewApi } = this.context;
            const { cellId, dayDate, todayRange } = this.props;
            const { textId } = this.state;
            const dayMeta = getDateMeta(dayDate, todayRange);
            // will ever be falsy?
            const text = options.listDayFormat ? dateEnv.format(dayDate, options.listDayFormat) : '';
            // will ever be falsy? also, BAD NAME "alt"
            const sideText = options.listDaySideFormat ? dateEnv.format(dayDate, options.listDaySideFormat) : '';
            const renderProps = Object.assign({ date: dateEnv.toDate(dayDate), view: viewApi, textId,
                text,
    ...
    class ListViewEventRow extends BaseComponent {
        render() {
            const { props, context } = this;
            const { options } = context;
            const { seg, timeHeaderId, eventHeaderId, dateHeaderId } = props;
            const timeFormat = options.eventTimeFormat || DEFAULT_TIME_FORMAT$1;
    ...
    function renderEventInnerContent(seg, context) {
        const interactiveAttrs = getSegAnchorAttrs(seg, context);
    ...
    */
    class ListView extends DateComponent {
        ...
        render() {
            const { props, context } = this;
            const { dayDates, dayRanges } = this.computeDateVars(props.dateProfile);
            const eventSegs = this.eventStoreToSegs(props.eventStore, props.eventUiBases, dayRanges);
        ...
        renderSegList(allSegs, dayDates) {
            const { theme, options } = this.context;
            const { timeHeaderId, eventHeaderId, dateHeaderIdRoot } = this.state;
            ...
            return (y(NowTimer, { unit: "day" }, (nowDate, todayRange) => {
                ...
                for (let dayIndex = 0; dayIndex < segsByDay.length; dayIndex += 1) {
                    ...
                    if (daySegs) { // sparse array, so might be undefined
                        const dayStr = formatDayString(dayDates[dayIndex]);
                        const dateHeaderId = dateHeaderIdRoot + '-' + dayStr;
                        // append a day header
        ...
        eventRangeToSegs(eventRange, dayRanges) {
            ...
            const { nextDayThreshold } = this.context.options;
    ...
    function buildLicenseWarning(context) {
        const key = context.options.schedulerLicenseKey;
        const currentUrl = typeof window !== 'undefined' ? window.location.href : '';
        if (!isImmuneUrl(currentUrl)) {
            const status = processLicenseKey(key, context.pluginHooks.premiumReleaseDate);
            if (status !== 'valid') {
    ...
    */
    ...
    function getScrollCanvasOrigin(scrollEl) {
        ...
        const edges = computeEdges(scrollEl); // TODO: pass in isRtl?
        return {
    ...
    function getScrollFromLeftEdge(el) {
        ...
        const computedStyles = window.getComputedStyle(el); // TODO: pass in isRtl instead?
    ...
    */
    class StickyScrolling {
        constructor(scrollEl, isRtl) {
            ...
            this.updateSize = () => {
                const { scrollEl } = this;
                const els = findElements(scrollEl, STICKY_SELECTOR);
                const elGeoms = this.queryElGeoms(els);
                const viewportWidth = scrollEl.clientWidth;
                assignStickyPositions(els, elGeoms, viewportWidth);
            };
        }
        queryElGeoms(els) {
            const { scrollEl, isRtl } = this;
            const canvasOrigin = getScrollCanvasOrigin(scrollEl);
            const elGeoms = [];
            for (const el of els) {
                const parentBound = translateRect(computeInnerRect(el.parentNode, true, true), // weird way to call this!!!
    ...
    function assignStickyPositions(els, elGeoms, viewportWidth) {
        els.forEach((el, i) => {
            const { textAlign, elWidth, parentBound } = elGeoms[i];
            const parentWidth = parentBound.right - parentBound.left;
            let left;
            if (textAlign === 'center' &&
    ...
    class ClippedScroller extends BaseComponent {
        ...
        render() {
            const { props, state, context } = this;
            const isScrollbarOnLeft = context.isRtl && getIsRtlScrollbarOnLeft();
            let overcomeLeft = 0;
            let overcomeRight = 0;
    ...
    */
    class ScrollListener {
        constructor(el) {
            ...
            for (const eventName of WHEEL_EVENT_NAMES) {
    ...
    class ScrollSyncer {
        ...
        destroy() {
            for (const scrollListener of this.scrollListeners) {
                scrollListener.destroy();
            }
        }
        bindScroller(el) {
            const { scrollEls, isVertical } = this;
            const scrollListener = new ScrollListener(el);
            const onScroll = (isWheel, isTouch) => {
                if (!this.isPaused) {
                    ...
                    if (this.masterEl === el) { // dealing with current
                        for (const otherEl of scrollEls) {
                            if (otherEl !== el) {
                                if (isVertical) {
    ...
    */
    class ScrollGrid extends BaseComponent {
        constructor() {
            ...
            this.handleRowHeightChange = (rowEl, isStable) => {
                const { rowUnstableMap, rowInnerMaxHeightMap } = this;
                if (!isStable) {
                    rowUnstableMap.set(rowEl, true);
                }
                else {
                    rowUnstableMap.delete(rowEl);
                    const innerMaxHeight = getRowInnerMaxHeight(rowEl);
        ...
        render() {
            const { props, state, context } = this;
            const { shrinkWidths } = state;
            const colGroupStats = this.compileColGroupStats(props.colGroups.map((colGroup) => [colGroup]));
            const microColGroupNodes = this.renderMicroColGroups(colGroupStats.map((stat, i) => [stat.cols, shrinkWidths[i]]));
            const classNames = getScrollGridClassNames(props.liquid, context);
            this.getDims();
            // TODO: make DRY
            const sectionConfigs = props.sections;
        ...
        renderChunk(sectionConfig, sectionIndex, colGroupStat, microColGroupNode, chunkConfig, chunkIndex, rowHeights, isHeader) {
            ...
            const { state } = this;
            const { scrollerClientWidths, scrollerClientHeights } = state;
            const [sectionCnt, chunksPerSection] = this.getDims();
            const index = sectionIndex * chunksPerSection + chunkIndex;
            const sideScrollIndex = (!this.context.isRtl || getIsRtlScrollbarOnLeft()) ? chunksPerSection - 1 : 0;
            const isVScrollSide = chunkIndex === sideScrollIndex;
            const isLastSection = sectionIndex === sectionCnt - 1;
            const forceXScrollbars = isLastSection && state.forceXScrollbars; // NOOOO can result in `null`
            const forceYScrollbars = isVScrollSide && state.forceYScrollbars; // NOOOO can result in `null`
            const allowXScrolling = colGroupStat && colGroupStat.allowXScrolling; // rename?
            const allowYScrolling = getAllowYScrolling(this.props, sectionConfig); // rename? do in section func?
            const chunkVGrow = getSectionHasLiquidHeight(this.props, sectionConfig); // do in section func?
            const expandRows = sectionConfig.expandRows && chunkVGrow;
            const tableMinWidth = (colGroupStat && colGroupStat.totalColMinWidth) || '';
            ...
            const overflowX = forceXScrollbars ? (isLastSection ? 'scroll' : 'scroll-hidden') :
                !allowXScrolling ? 'hidden' :
                    (isLastSection ? 'auto' : 'scroll-hidden');
            const overflowY = forceYScrollbars ? (isVScrollSide ? 'scroll' : 'scroll-hidden') :
        ...
        computeShrinkWidths() {
            const colGroupStats = this.compileColGroupStats(this.props.colGroups.map((colGroup) => [colGroup]));
            const [sectionCnt, chunksPerSection] = this.getDims();
            const cnt = sectionCnt * chunksPerSection;
            const shrinkWidths = [];
            colGroupStats.forEach((colGroupStat, i) => {
                if (colGroupStat.hasShrinkCol) {
                    const chunkEls = this.chunkElRefs.collect(i, cnt, chunksPerSection); // in one col
        ...
        computeSectionRowMaxHeights() {
            ...
            const [sectionCnt, chunksPerSection] = this.getDims();
            const sectionRowMaxHeights = [];
            for (let sectionI = 0; sectionI < sectionCnt; sectionI += 1) {
                const sectionConfig = this.props.sections[sectionI];
                const assignableHeights = []; // chunk, row
                if (sectionConfig && sectionConfig.syncRowHeights) {
                    const rowHeightsByChunk = [];
                    for (let chunkI = 0; chunkI < chunksPerSection; chunkI += 1) {
                        const index = sectionI * chunksPerSection + chunkI;
                        let rowHeights = [];
                        const chunkEl = this.chunkElRefs.currentMap[index];
                        if (chunkEl) {
                            rowHeights = findElements(chunkEl, '.fc-scrollgrid-sync-table tr').map((rowEl) => {
                                const max = getRowInnerMaxHeight(rowEl);
                    ...
                    const rowCnt = rowHeightsByChunk[0].length;
                    let isEqualRowCnt = true;
                    for (let chunkI = 1; chunkI < chunksPerSection; chunkI += 1) {
                        const isOuterContent = sectionConfig.chunks[chunkI] && sectionConfig.chunks[chunkI].outerContent !== undefined; // can be null
                    ...
                    if (!isEqualRowCnt) {
                        const chunkHeightSums = [];
                        for (let chunkI = 0; chunkI < chunksPerSection; chunkI += 1) {
                            chunkHeightSums.push(sumNumbers(rowHeightsByChunk[chunkI]) + rowHeightsByChunk[chunkI].length);
                        }
                        const maxTotalSum = Math.max(...chunkHeightSums);
                        for (let chunkI = 0; chunkI < chunksPerSection; chunkI += 1) {
                            const rowInChunkCnt = rowHeightsByChunk[chunkI].length;
                            const rowInChunkTotalHeight = maxTotalSum - rowInChunkCnt; // subtract border
                            // height of non-first row. we do this to avoid rounding, because it's unreliable within a table
                            const rowInChunkHeightOthers = Math.floor(rowInChunkTotalHeight / rowInChunkCnt);
                            // whatever is leftover goes to the first row
                            const rowInChunkHeightFirst = rowInChunkTotalHeight - rowInChunkHeightOthers * (rowInChunkCnt - 1);
                            const rowInChunkHeights = [];
                            let row = 0;
                        ...
                        for (let row = 0; row < rowCnt; row += 1) {
                            const rowHeightsAcrossChunks = [];
                            for (let chunkI = 0; chunkI < chunksPerSection; chunkI += 1) {
                                const h = rowHeightsByChunk[chunkI][row];
                                if (h != null) { // protect against outerContent
                                    rowHeightsAcrossChunks.push(h);
                                }
                            }
                            const maxHeight = Math.max(...rowHeightsAcrossChunks);
                            for (let chunkI = 0; chunkI < chunksPerSection; chunkI += 1) {
        ...
        computeScrollerDims() {
            const scrollbarWidth = getScrollbarWidths();
            const [sectionCnt, chunksPerSection] = this.getDims();
            const sideScrollI = (!this.context.isRtl || getIsRtlScrollbarOnLeft()) ? chunksPerSection - 1 : 0;
            const lastSectionI = sectionCnt - 1;
            const currentScrollers = this.clippedScrollerRefs.currentMap;
            const scrollerEls = this.scrollerElRefs.currentMap;
            let forceYScrollbars = false;
            let forceXScrollbars = false;
            const scrollerClientWidths = {};
            const scrollerClientHeights = {};
            for (let sectionI = 0; sectionI < sectionCnt; sectionI += 1) { // along edge
                const index = sectionI * chunksPerSection + sideScrollI;
                const scroller = currentScrollers[index];
                if (scroller && scroller.needsYScrolling()) {
                    forceYScrollbars = true;
                    break;
                }
            ...
            for (let chunkI = 0; chunkI < chunksPerSection; chunkI += 1) { // along last row
                const index = lastSectionI * chunksPerSection + chunkI;
                const scroller = currentScrollers[index];
            ...
            for (let sectionI = 0; sectionI < sectionCnt; sectionI += 1) {
                for (let chunkI = 0; chunkI < chunksPerSection; chunkI += 1) {
                    const index = sectionI * chunksPerSection + chunkI;
                    const scrollerEl = scrollerEls[index];
                    if (scrollerEl) {
                        // TODO: weird way to get this. need harness b/c doesn't include table borders
                        const harnessEl = scrollerEl.parentNode;
        ...
        updateStickyScrolling() {
            const { isRtl } = this.context;
            const argsByKey = this.scrollerElRefs.getAll().map((scrollEl) => [scrollEl, isRtl]);
        ...
        updateScrollSyncers() {
            const [sectionCnt, chunksPerSection] = this.getDims();
            const cnt = sectionCnt * chunksPerSection;
            const scrollElsBySection = {};
            const scrollElsByColumn = {};
            const scrollElMap = this.scrollerElRefs.currentMap;
            for (let sectionI = 0; sectionI < sectionCnt; sectionI += 1) {
                const startIndex = sectionI * chunksPerSection;
                const endIndex = startIndex + chunksPerSection;
        ...
        getChunkConfigByIndex(index) {
            const chunksPerSection = this.getDims()[1];
            const sectionI = Math.floor(index / chunksPerSection);
            const chunkI = index % chunksPerSection;
            const sectionConfig = this.props.sections[sectionI];
            return sectionConfig && sectionConfig.chunks[chunkI];
        }
        forceScrollLeft(col, scrollLeft) {
            const scrollSyncer = this.scrollSyncersByColumn[col];
            if (scrollSyncer) {
                scrollSyncer.forceScrollLeft(scrollLeft);
            }
        }
        forceScrollTop(sectionI, scrollTop) {
            const scrollSyncer = this.scrollSyncersBySection[sectionI];
            if (scrollSyncer) {
                scrollSyncer.forceScrollTop(scrollTop);
            }
        }
        _handleChunkEl(chunkEl, key) {
            const chunkConfig = this.getChunkConfigByIndex(parseInt(key, 10));
        ...
        getDims() {
            ...
            const chunksPerSection = sectionCnt ? this.props.sections[0].chunks.length : 0;
    ...
    function getRowInnerMaxHeight(rowEl) {
        const innerHeights = findElements(rowEl, '.fc-scrollgrid-sync-inner').map(getElHeight);
    ...
    function renderMacroColGroup(colGroupStats, shrinkWidths) {
        const children = colGroupStats.map((colGroupStat, i) => {
    ...
    function compileColGroupStat(colGroupConfig) {
        const totalColWidth = sumColProp(colGroupConfig.cols, 'width'); // excludes "shrink"
        const totalColMinWidth = sumColProp(colGroupConfig.cols, 'minWidth');
        const hasShrinkCol = hasShrinkWidth(colGroupConfig.cols);
        const allowXScrolling = colGroupConfig.width !== 'shrink' && Boolean(totalColWidth || totalColMinWidth || hasShrinkCol);
        return {
            hasShrinkCol,
            totalColWidth,
            totalColMinWidth,
    ...
    function freezeScrollgridWidths() {
        const els = findElements(document.body, '.fc-scrollgrid');
    ...
    function buildTimelineDateProfile(dateProfile, dateEnv, allOptions, dateProfileGenerator) {
        ...
        const input = allOptions.slotLabelFormat;
        const rawFormats = Array.isArray(input) ? input :
            (input != null) ? [input] :
        ...
        */
        const rawSnapDuration = allOptions.snapDuration;
        let snapDuration;
        let snapsPerSlot;
        if (rawSnapDuration) {
        ...
        // more...
        const timeWindowMs = asRoughMs(dateProfile.slotMaxTime) - asRoughMs(dateProfile.slotMinTime);
    ...
    */
    function normalizeRange(range, tDateProfile, dateEnv) {
        if (!tDateProfile.isTimeScale) {
            range = computeVisibleDayRange(range);
            if (tDateProfile.largeUnit) {
                const dayRange = range; // preserve original result
                range = {
    ...
    function ensureSlotDuration(tDateProfile, dateProfile, dateEnv) {
        ...
        if (!slotDuration) {
            ...
            for (const input of STOCK_SUB_DURATIONS) {
    ...
    function currentRangeAs(unit, dateProfile, dateEnv) {
        const range = dateProfile.currentRange;
        let res = null;
        if (unit === 'years') {
    ...
    function buildIsWeekStarts(tDateProfile, dateEnv) {
        const { slotDates, emphasizeWeeks } = tDateProfile;
        let prevWeekNumber = null;
        const isWeekStarts = [];
        for (const slotDate of slotDates) {
            const weekNumber = dateEnv.computeWeekNumber(slotDate);
            const isWeekStart = emphasizeWeeks && (prevWeekNumber !== null) && (prevWeekNumber !== weekNumber);
    ...
    function buildCellRows(tDateProfile, dateEnv) {
        const slotDates = tDateProfile.slotDates;
        const formats = tDateProfile.headerFormats;
        const cellRows = formats.map(() => []); // indexed by row,col
        const slotAsDays = asCleanDays(tDateProfile.slotDuration);
        const guessedSlotUnit = slotAsDays === 7 ? 'week' :
            slotAsDays === 1 ? 'day' :
                null;
        // specifically for navclicks
        const rowUnitsFromFormats = formats.map((format) => (format.getLargestUnit ? format.getLargestUnit() : null));
        // builds cellRows and slotCells
        for (let i = 0; i < slotDates.length; i += 1) {
            const date = slotDates[i];
            const isWeekStart = tDateProfile.isWeekStarts[i];
            for (let row = 0; row < formats.length; row += 1) {
                const format = formats[row];
                const rowCells = cellRows[row];
                const leadingCell = rowCells[rowCells.length - 1];
                const isLastRow = row === formats.length - 1;
                const isSuperRow = formats.length > 1 && !isLastRow; // more than one row and not the last
                let newCell = null;
                const rowUnit = rowUnitsFromFormats[row] || (isLastRow ? guessedSlotUnit : null);
                if (isSuperRow) {
                    const text = dateEnv.format(date, format);
    ...
    class TimelineHeaderTh extends BaseComponent {
        ...
        render() {
            const { props, context } = this;
            const { dateEnv, options } = context;
            const { cell, dateProfile, tDateProfile } = props;
            // the cell.rowUnit is f'd
            // giving 'month' for a 3-day view
            // workaround: to infer day, do NOT time
            const dateMeta = getDateMeta(cell.date, props.todayRange, props.nowDate, dateProfile);
            const renderProps = this.refineRenderProps({
    ...
    class TimelineHeaderRows extends BaseComponent {
        render() {
            const { dateProfile, tDateProfile, rowInnerHeights, todayRange, nowDate } = this.props;
            const { cellRows } = tDateProfile;
            return (y(_, null, cellRows.map((rowCells, rowLevel) => {
                const isLast = rowLevel === cellRows.length - 1;
                const isChrono = tDateProfile.isTimeScale && isLast; // the final row, with times?
                const classNames = [
    ...
    class TimelineCoords {
        ...
        dateToCoord(date) {
            const { tDateProfile } = this;
            const snapCoverage = this.computeDateSnapCoverage(date);
            const slotCoverage = snapCoverage / tDateProfile.snapsPerSlot;
            let slotIndex = Math.floor(slotCoverage);
            slotIndex = Math.min(slotIndex, tDateProfile.slotCnt - 1);
            const partial = slotCoverage - slotIndex;
            const { innerCoordCache, outerCoordCache } = this;
            if (this.isRtl) {
        ...
        durationToCoord(duration) {
            const { dateProfile, tDateProfile, dateEnv, isRtl } = this;
            let coord = 0;
            if (dateProfile) {
    ...
    function computeDateSnapCoverage(date, tDateProfile, dateEnv) {
        const snapDiff = dateEnv.countDurationsBetween(tDateProfile.normalizedRange.start, date, tDateProfile.snapDuration);
        if (snapDiff < 0) {
            return 0;
        }
    ...
    class TimelineHeader extends BaseComponent {
        ...
        render() {
            ...
            const timerUnit = greatestDurationDenominator(props.tDateProfile.slotDuration).unit;
            // WORKAROUND: make ignore slatCoords when out of sync with dateProfile
            const slatCoords = props.slatCoords && props.slatCoords.dateProfile === props.dateProfile ? props.slatCoords : null;
    ...
    class TimelineSlatCell extends BaseComponent {
        render() {
            const { props, context } = this;
            const { dateEnv, options, theme } = context;
            const { date, tDateProfile, isEm } = props;
            const dateMeta = getDateMeta(props.date, props.todayRange, props.nowDate, props.dateProfile);
            const renderProps = Object.assign(Object.assign({ date: dateEnv.toDate(props.date) }, dateMeta), { view: context.viewApi });
    ...
    class TimelineSlatsBody extends BaseComponent {
        render() {
            const { props } = this;
            const { tDateProfile, cellElRefs } = props;
            const { slotDates, isWeekStarts } = tDateProfile;
            const isDay = !tDateProfile.isTimeScale && !tDateProfile.largeUnit;
            return (y("tbody", null,
    ...
    class TimelineSlats extends BaseComponent {
        constructor() {
            ...
            this.cellElRefs = new RefMap();
            this.handleScrollRequest = (request) => {
                const { onScrollLeftRequest } = this.props;
                const { coords } = this;
                if (onScrollLeftRequest && coords) {
                    if (request.time) {
                        const scrollLeft = coords.coordFromLeft(coords.durationToCoord(request.time));
        ...
        positionToHit(leftPosition) {
            const { outerCoordCache } = this.coords;
            const { dateEnv, isRtl } = this.context;
            const { tDateProfile } = this.props;
            const slatIndex = outerCoordCache.leftToIndex(leftPosition);
            if (slatIndex != null) {
                // somewhat similar to what TimeGrid does. consolidate?
                const slatWidth = outerCoordCache.getWidth(slatIndex);
                const partial = isRtl ?
                ...
                const end = dateEnv.add(start, tDateProfile.snapDuration);
                return {
                    dateSpan: {
                        range: { start, end },
    ...
    function computeSegHCoords(segs, minWidth, timelineCoords) {
        ...
        if (timelineCoords) {
            for (const seg of segs) {
                const res = timelineCoords.rangeToCoords(seg);
                const start = Math.round(res.start); // for barely-overlapping collisions
    ...
    strictOrder, maxStackCnt) {
        const segInputs = [];
        const crudePlacements = []; // when we don't know dims
        for (let i = 0; i < segs.length; i += 1) {
            const seg = segs[i];
            const instanceId = seg.eventRange.instance.instanceId;
            const height = eventInstanceHeights[instanceId];
            const hcoords = segHCoords[i];
            if (height && hcoords) {
                segInputs.push({
                    index: i,
                    span: hcoords,
        ...
        const hiddenEntries = hierarchy.addSegs(segInputs);
        const hiddenPlacements = hiddenEntries.map((entry) => ({
            seg: segs[entry.index],
            hcoords: entry.span,
            top: null,
        }));
        const hiddenGroups = groupIntersectingEntries(hiddenEntries);
        const moreLinkInputs = [];
        const moreLinkCrudePlacements = [];
        const extractSeg = (entry) => segs[entry.index];
        for (let i = 0; i < hiddenGroups.length; i += 1) {
            const hiddenGroup = hiddenGroups[i];
            const sortedSegs = hiddenGroup.entries.map(extractSeg);
            const height = moreLinkHeights[buildIsoString(computeEarliestSegStart(sortedSegs))]; // not optimal :(
            if (height != null) {
        ...
        const visibleRects = hierarchy.toRects();
        const visiblePlacements = [];
        let maxHeight = 0;
        for (const rect of visibleRects) {
            const segIndex = rect.index;
    ...
    class TimelineLaneBg extends BaseComponent {
        render() {
            const { props } = this;
            const highlightSeg = [].concat(props.eventResizeSegs, props.dateSelectionSegs);
        ...
        renderSegs(segs, timelineCoords, fillType) {
            const { todayRange, nowDate } = this.props;
            const { isRtl } = this.context;
            const segHCoords = computeSegHCoords(segs, 0, timelineCoords);
            const children = segs.map((seg, i) => {
                const hcoords = segHCoords[i];
                const hStyle = coordsToCss(hcoords, isRtl);
    ...
    class TimelineLaneSlicer extends Slicer {
        sliceRange(origRange, dateProfile, dateProfileGenerator, tDateProfile, dateEnv) {
            const normalRange = normalizeRange(origRange, tDateProfile, dateEnv);
            const segs = [];
            // protect against when the span is entirely in an invalid date region
            if (computeDateSnapCoverage(normalRange.start, tDateProfile, dateEnv)
                < computeDateSnapCoverage(normalRange.end, tDateProfile, dateEnv)) {
                // intersect the footprint's range with the grid's range
                const slicedRange = intersectRanges(normalRange, tDateProfile.normalizedRange);
                if (slicedRange) {
                    segs.push({
    ...
    class TimelineLaneMoreLink extends BaseComponent {
        render() {
            const { props, context } = this;
            const { hiddenSegs, placement, resourceId } = props;
            const { top, hcoords } = placement;
            const isVisible = hcoords && top !== null;
            const hStyle = coordsToCss(hcoords, context.isRtl);
            const extraDateSpan = resourceId ? { resourceId } : {};
    ...
    class TimelineLane extends BaseComponent {
        ...
        render() {
            const { props, state, context } = this;
            const { options } = context;
            const { dateProfile, tDateProfile } = props;
            const slicedProps = this.slicer.sliceProps(props, dateProfile, tDateProfile.isTimeScale ? null : props.nextDayThreshold, context, // wish we didn't have to pass in the rest of the args...
            dateProfile, context.dateProfileGenerator, tDateProfile, context.dateEnv);
            const mirrorSegs = (slicedProps.eventDrag ? slicedProps.eventDrag.segs : null) ||
                (slicedProps.eventResize ? slicedProps.eventResize.segs : null) ||
                [];
            const fgSegs = this.sortEventSegs(slicedProps.fgEventSegs, options.eventOrder);
            const fgSegHCoords = computeSegHCoords(fgSegs, options.eventMinWidth, props.timelineCoords);
            const [fgPlacements, fgHeight] = computeFgSegPlacements(fgSegs, fgSegHCoords, state.eventInstanceHeights, state.moreLinkHeights, options.eventOrderStrict, options.eventMaxStack);
            const isForcedInvisible = // TODO: more convenient
        ...
        renderFgSegs(segPlacements, isForcedInvisible, isDragging, isResizing, isDateSelecting) {
            const { harnessElRefs, moreElRefs, props, context } = this;
            const isMirror = isDragging || isResizing || isDateSelecting;
            return (y(_, null, segPlacements.map((segPlacement) => {
                const { seg, hcoords, top } = segPlacement;
                if (Array.isArray(seg)) { // a more-link
                    const isoStr = buildIsoString(computeEarliestSegStart(seg));
                ...
                const instanceId = seg.eventRange.instance.instanceId;
                const isVisible = isMirror || Boolean(!isForcedInvisible[instanceId] && hcoords && top !== null);
                const hStyle = coordsToCss(hcoords, context.isRtl);
    ...
    function buildMirrorPlacements(mirrorSegs, timelineCoords, fgPlacements) {
        ...
        const topsByInstanceId = buildAbsoluteTopHash(fgPlacements); // TODO: cache this at first render?
        return mirrorSegs.map((seg) => ({
            seg,
    ...
    class TimelineGrid extends DateComponent {
        ...
        // ------------------------------------------------------------------------------------------
        queryHit(positionLeft, positionTop, elWidth, elHeight) {
            const slats = this.slatsRef.current;
            const slatHit = slats.positionToHit(positionLeft);
            if (slatHit) {
                return {
                    dateProfile: this.props.dateProfile,
    ...
    class TimelineView extends DateComponent {
        constructor() {
            ...
            this.handleScrollLeftRequest = (scrollLeft) => {
                const scrollGrid = this.scrollGridRef.current;
        ...
        render() {
            ...
            const { slotMinWidth } = options;
            const slatCols = buildSlatCols(tDateProfile, slotMinWidth || this.computeFallbackSlotMinWidth(tDateProfile));
            const sections = [
                {
                    type: 'header',
                    key: 'header',
    ...
    */
    function parseResource(raw, parentId = '', store, context) {
        const { refined, extra } = refineProps(raw, RESOURCE_REFINERS);
        const resource = {
        ...
        if (store[resource.id]) ;
            ...
            if (refined.children) {
                for (const childInput of refined.children) {
    ...
    */
    ...
    class ResourceApi {
        ...
        sync(oldResource) {
            const context = this._context;
            const resourceId = oldResource.id;
        ...
        remove() {
            const context = this._context;
            const internalResource = this._resource;
            const resourceId = internalResource.id;
            context.dispatch({
        ...
        getParent() {
            const context = this._context;
            const parentId = this._resource.parentId;
            if (parentId) {
        ...
        getChildren() {
            const thisResourceId = this._resource.id;
            const context = this._context;
            const { resourceStore } = context.getCurrentData();
            const childApis = [];
            for (const resourceId in resourceStore) {
        ...
        */
        getEvents() {
            const thisResourceId = this._resource.id;
            const context = this._context;
            const { defs, instances } = context.getCurrentData().eventStore;
            const eventApis = [];
            for (const instanceId in instances) {
                const instance = instances[instanceId];
                const def = defs[instance.defId];
        ...
        toPlainObject(settings = {}) {
            const internal = this._resource;
            const { ui } = internal;
            const publicId = this.id;
            const res = {};
            if (publicId) {
                res.id = publicId;
            }
    ...
    class ResourceDayHeader extends BaseComponent {
        ...
        render() {
            const { props, context } = this;
            const dateFormat = this.buildDateFormat(context.options.dayHeaderFormat, props.datesRepDistinctDays, props.dates.length);
            return (y(NowTimer, { unit: "day" }, (nowDate, todayRange) => {
        ...
        renderResourceRow(resources, date) {
            const resourceCells = resources.map((resource) => (y(ResourceCell, { key: resource.id, resource: resource, colSpan: 1, date: date })));
        ...
        renderDateCell(date, dateFormat, todayRange, colSpan, resource, isSticky) {
            const { props } = this;
            const keyPostfix = resource ? `:${resource.id}` : '';
            const extraRenderProps = resource ? { resource: new ResourceApi(this.context, resource) } : {};
            const extraDataAttrs = resource ? { 'data-resource-id': resource.id } : {};
    ...
    class AbstractResourceDayTableModel {
        ...
        buildCells() {
            const { rowCnt, dayTableModel, resources } = this;
            const rows = [];
            for (let row = 0; row < rowCnt; row += 1) {
                const rowCells = [];
                for (let dateCol = 0; dateCol < dayTableModel.colCnt; dateCol += 1) {
                    for (let resourceCol = 0; resourceCol < resources.length; resourceCol += 1) {
                        const resource = resources[resourceCol];
                        const extraRenderProps = { resource: new ResourceApi(this.context, resource) };
                        const extraDataAttrs = { 'data-resource-id': resource.id };
                        const extraClassNames = ['fc-resource'];
                        const extraDateSpan = { resourceId: resource.id };
                        const date = dayTableModel.cells[row][dateCol].date;
        ...
        */
        computeColRanges(dateStartI, dateEndI, resourceI) {
            ...
            for (let i = dateStartI; i <= dateEndI; i += 1) {
                const col = this.computeCol(i, resourceI);
                segs.push({
                    firstCol: col,
                    lastCol: col,
    ...
    class VResourceJoiner {
        ...
        */
        joinProps(propSets, resourceDayTable) {
            const dateSelectionSets = [];
            const businessHoursSets = [];
            const fgEventSets = [];
            const bgEventSets = [];
            const eventDrags = [];
            const eventResizes = [];
            let eventSelection = '';
            const keys = resourceDayTable.resourceIndex.ids.concat(['']); // add in the all-resource key
            for (const key of keys) {
                const props = propSets[key];
        ...
        joinSegs(resourceDayTable, ...segGroups) {
            const resourceCnt = resourceDayTable.resources.length;
            const transformedSegs = [];
            for (let i = 0; i < resourceCnt; i += 1) {
                for (const seg of segGroups[i]) {
                    transformedSegs.push(...this.transformSeg(seg, resourceDayTable, i));
                }
                for (const seg of segGroups[resourceCnt]) { // one beyond. the all-resource
                    transformedSegs.push(...this.transformSeg(seg, resourceDayTable, i));
                }
            }
        ...
        joinInteractions(resourceDayTable, ...interactions) {
            ...
            for (let i = 0; i < resourceCnt; i += 1) {
                ...
                if (interaction) {
                    ...
                    for (const seg of interaction.segs) {
                        transformedSegs.push(...this.transformSeg(seg, resourceDayTable, i));
                    }
                ...
                if (interactions[resourceCnt]) { // one beyond. the all-resource
                    for (const seg of interactions[resourceCnt].segs) {
                        transformedSegs.push(...this.transformSeg(seg, resourceDayTable, i));
                    }
                }
            }
    ...
    */
    class VResourceSplitter extends Splitter {
        getKeyInfo(props) {
            const { resourceDayTableModel } = props;
            const hash = mapHash(resourceDayTableModel.resourceIndex.indicesById, (i) => resourceDayTableModel.resources[i]); // :(
            hash[''] = {};
            return hash;
        }
        getKeysForDateSpan(dateSpan) {
        ...
        getKeysForEventDef(eventDef) {
            const resourceIds = eventDef.resourceIds;
    ...
    */
    ...
    function buildRowNodes(resourceStore, groupSpecs, orderSpecs, isVGrouping, expansions, expansionDefault) {
        const complexNodes = buildHierarchy(resourceStore, isVGrouping ? -1 : 1, groupSpecs, orderSpecs);
        const flatNodes = [];
    ...
    function flattenNodes(complexNodes, res, isVGrouping, rowSpans, depth, expansions, expansionDefault) {
        for (let i = 0; i < complexNodes.length; i += 1) {
            const complexNode = complexNodes[i];
            const group = complexNode.group;
            if (group) {
                if (isVGrouping) {
                    const firstRowIndex = res.length;
                    const rowSpanIndex = rowSpans.length;
                    flattenNodes(complexNode.children, res, isVGrouping, rowSpans.concat(0), depth, expansions, expansionDefault);
                    if (firstRowIndex < res.length) {
                        const firstRow = res[firstRowIndex];
                        const firstRowSpans = firstRow.rowSpans = firstRow.rowSpans.slice();
                        firstRowSpans[rowSpanIndex] = res.length - firstRowIndex;
                    }
                }
                else {
                    const id = group.spec.field + ':' + group.value;
                    const isExpanded = expansions[id] != null ? expansions[id] : expansionDefault;
                ...
                const id = complexNode.resource.id;
                const isExpanded = expansions[id] != null ? expansions[id] : expansionDefault;
                res.push({
                    id,
                    rowSpans,
    ...
    function buildHierarchy(resourceStore, maxDepth, groupSpecs, orderSpecs) {
        const resourceNodes = buildResourceNodes(resourceStore, orderSpecs);
        const builtNodes = [];
        for (const resourceId in resourceNodes) {
            const resourceNode = resourceNodes[resourceId];
    ...
    function buildResourceNodes(resourceStore, orderSpecs) {
        const nodeHash = {};
        for (const resourceId in resourceStore) {
            const resource = resourceStore[resourceId];
            nodeHash[resourceId] = {
                resource,
        ...
        for (const resourceId in resourceStore) {
            ...
            if (resource.parentId) {
                const parentNode = nodeHash[resource.parentId];
                if (parentNode) {
    ...
    function insertResourceNode(resourceNode, nodes, groupSpecs, depth, maxDepth, orderSpecs) {
        if (groupSpecs.length && (maxDepth === -1 || depth <= maxDepth)) {
            const groupNode = ensureGroupNodes(resourceNode, nodes, groupSpecs[0]);
    ...
    function ensureGroupNodes(resourceNode, nodes, groupSpec) {
        const groupValue = resourceNode.resourceFields[groupSpec.field];
        let groupNode;
        let newGroupIndex;
        // find an existing group that matches, or determine the position for a new group
        if (groupSpec.order) {
            for (newGroupIndex = 0; newGroupIndex < nodes.length; newGroupIndex += 1) {
                const node = nodes[newGroupIndex];
                if (node.group) {
                    const cmp = flexibleCompare(groupValue, node.group.value) * groupSpec.order;
                    if (cmp === 0) {
                        groupNode = node;
                        break;
                    }
    ...
    function insertResourceNodeInSiblings(resourceNode, siblings, orderSpecs) {
        ...
        for (i = 0; i < siblings.length; i += 1) {
            const cmp = compareByFieldSpecs(siblings[i].resourceFields, resourceNode.resourceFields, orderSpecs); // TODO: pass in ResourceApi?
    ...
    function buildResourceFields(resource) {
        const obj = Object.assign(Object.assign(Object.assign({}, resource.extendedProps), resource.ui), resource);
        delete obj.ui;
    ...
    */
    ...
    function massageEventDragMutation(eventMutation, hit0, hit1) {
        const resource0 = hit0.dateSpan.resourceId;
        const resource1 = hit1.dateSpan.resourceId;
    ...
    */
    function applyEventDefMutation(eventDef, mutation, context) {
        const resourceMutation = mutation.resourceMutation;
        if (resourceMutation && computeResourceEditable(eventDef, context)) {
            const index = eventDef.resourceIds.indexOf(resourceMutation.matchResourceId);
            if (index !== -1) {
                const resourceIds = eventDef.resourceIds.slice(); // copy
    ...
    */
    function computeResourceEditable(eventDef, context) {
        ...
        if (resourceEditable == null) {
            const source = eventDef.sourceId && context.getCurrentData().eventSources[eventDef.sourceId];
            if (source) {
    ...
    function transformEventDrop(mutation, context) {
        const { resourceMutation } = mutation;
        if (resourceMutation) {
            const { calendarApi } = context;
            return {
    ...
    function filterResources(resourceStore, doFilterResourcesWithEvents, eventStore, activeRange) {
        if (doFilterResourcesWithEvents) {
            const instancesInRange = filterEventInstancesInRange(eventStore.instances, activeRange);
            const hasEvents = computeHasEvents(instancesInRange, eventStore.defs);
    ...
    function computeHasEvents(eventInstances, eventDefs) {
        const hasEvents = {};
        for (const instanceId in eventInstances) {
            const instance = eventInstances[instanceId];
            for (const resourceId of eventDefs[instance.defId].resourceIds) {
    ...
    */
    function transformIsDraggable(val, eventDef, eventUi, context) {
        if (!val) {
            const state = context.getCurrentData();
            const viewSpec = state.viewSpecs[state.currentViewType];
    ...
    function injectResourceEventUi(origEventUi, eventDef, resourceEventUis) {
        ...
        for (const resourceId of eventDef.resourceIds) {
    ...
    const defs = []; // TODO: use plugin system
    ...
    function parseResourceSource(input) {
        ...
        if (inputObj) {
            const { refined, extra } = refineProps(inputObj, RESOURCE_SOURCE_REFINERS);
            warnUnknownProps(extra);
            const metaRes = buildResourceSourceMeta(refined);
            if (metaRes) {
                return {
                    _raw: input,
                    sourceId: guid(),
    ...
    function buildResourceSourceMeta(refined) {
        const defs = getResourceSourceDefs();
        for (let i = defs.length - 1; i >= 0; i -= 1) { // later-added plugins take precedence
            const def = defs[i];
            const meta = def.parseMeta(refined);
            if (meta) {
    ...
    function reduceResourceSource(source, action, context) {
        const { options, dateProfile } = context;
        if (!source || !action) {
    ...
    function fetchSource(source, fetchRange, context) {
        const sourceDef = getResourceSourceDef(source.sourceDefId);
        const fetchId = guid();
        sourceDef.fetch({
    ...
    function removeResource(existingStore, resourceId) {
        const newStore = Object.assign({}, existingStore);
        delete newStore[resourceId];
        // promote children
        for (const childResourceId in newStore) { // a child, *maybe* but probably not
    ...
    function setResourceProp(existingStore, resourceId, name, value) {
        const existingResource = existingStore[resourceId];
        // TODO: sanitization
        if (existingResource) {
    ...
    function reduceResources(state, action, context) {
        const resourceSource = reduceResourceSource(state && state.resourceSource, action, context);
        const resourceStore = reduceResourceStore(state && state.resourceStore, action, resourceSource, context);
        const resourceEntityExpansions = reduceResourceEntityExpansions(state && state.resourceEntityExpansions, action);
        return {
            resourceSource,
            resourceStore,
    ...
    function transformDateSelectionJoin(hit0, hit1) {
        const resourceId0 = hit0.dateSpan.resourceId;
        const resourceId1 = hit1.dateSpan.resourceId;
    ...
    CalendarImpl.prototype.addResource = function (input, scrollTo = true) {
        ...
        const resourceApi = new ResourceApi(currentState, resource);
    ...
    CalendarImpl.prototype.getResourceById = function (id) {
        id = String(id);
        const currentState = this.getCurrentData(); // eslint-disable-line react/no-this-in-sfc
        if (currentState.resourceStore) { // guard against calendar with no resource functionality
            const rawResource = currentState.resourceStore[id];
            if (rawResource) {
    ...
    CalendarImpl.prototype.getResources = function () {
        const currentState = this.getCurrentData();
        const { resourceStore } = currentState;
        const resourceApis = [];
    ...
    function isPropsValidWithResources(combinedProps, context) {
        const splitter = new ResourceSplitter();
        const sets = splitter.splitProps(Object.assign(Object.assign({}, combinedProps), { resourceStore: context.getCurrentData().resourceStore }));
        for (const resourceId in sets) {
    ...
    function filterConstraints(resourceId, constraints) {
        return constraints.map((constraint) => {
            const defs = constraint.defs;
            if (defs) { // we are dealing with an EventStore
                // if any of the events define constraints to resources that are NOT this resource,
                // then this resource is unconditionally prohibited, which is what a `false` value does.
                for (const defId in defs) {
                    const resourceIds = defs[defId].resourceIds;
    ...
    function handleResources(newSourceInput, context) {
        const oldSourceInput = context.getCurrentData().resourceSource._raw;
    ...
    EventImpl.prototype.getResources = function () {
        const { calendarApi } = this._context;
    ...
    class ResourceDayTableJoiner extends VResourceJoiner {
        transformSeg(seg, resourceDayTableModel, resourceI) {
            const colRanges = resourceDayTableModel.computeColRanges(seg.firstCol, seg.lastCol, resourceI);
    ...
    class ResourceDayTable extends DateComponent {
        constructor() {
            ...
            this.isHitComboAllowed = (hit0, hit1) => {
                const allowAcrossResources = this.props.resourceDayTableModel.dayTableModel.colCnt === 1;
                return allowAcrossResources || hit0.dateSpan.resourceId === hit1.dateSpan.resourceId;
            };
        }
        render() {
            const { props, context } = this;
            const { resourceDayTableModel, nextDayThreshold, dateProfile } = props;
            ...
            const slicedProps = mapHash(this.slicers, (slicer, resourceId) => slicer.sliceProps(splitProps[resourceId], dateProfile, nextDayThreshold, context, resourceDayTableModel.dayTableModel));
    ...
    function buildResourceDayTableModel(dateProfile, dateProfileGenerator, resources, datesAboveResources, context) {
        const dayTable = buildDayTableModel(dateProfile, dateProfileGenerator);
    ...
    class ResourceDayTimeCols extends DateComponent {
        constructor() {
            ...
            this.timeColsRef = d();
            this.isHitComboAllowed = (hit0, hit1) => {
                const allowAcrossResources = this.dayRanges.length === 1;
                return allowAcrossResources || hit0.dateSpan.resourceId === hit1.dateSpan.resourceId;
            };
        }
        render() {
            const { props, context } = this;
            const { dateEnv, options } = context;
            const { dateProfile, resourceDayTableModel } = props;
            const dayRanges = this.dayRanges = this.buildDayRanges(resourceDayTableModel.dayTableModel, dateProfile, dateEnv);
            const splitProps = this.splitter.splitProps(props);
            this.slicers = mapHash(splitProps, (split, resourceId) => this.slicers[resourceId] || new DayTimeColsSlicer());
            const slicedProps = mapHash(this.slicers, (slicer, resourceId) => slicer.sliceProps(splitProps[resourceId], dateProfile, null, context, dayRanges));
        ...
        buildNowIndicatorSegs(date) {
            const nonResourceSegs = this.slicers[''].sliceNowDate(date, this.props.dateProfile, this.context.options.nextDayThreshold, this.context, this.dayRanges);
    ...
    function buildResourceTimeColsModel(dateProfile, dateProfileGenerator, resources, datesAboveResources, context) {
        const dayTable = buildTimeColsModel(dateProfile, dateProfileGenerator);
        return datesAboveResources ?
            new DayResourceTableModel(dayTable, resources, context) :
            new ResourceDayTableModel(dayTable, resources, context);
    }
    ...
    */
    ...
    class SpreadsheetRow extends BaseComponent {
        render() {
            const { props } = this;
            const { resource, rowSpans, depth } = props;
            const resourceFields = buildResourceFields(resource); // slightly inefficient. already done up the call stack
    ...
    class SpreadsheetGroupRow extends BaseComponent {
        ...
        render() {
            const { props, context } = this;
            const renderProps = { groupValue: props.group.value, view: context.viewApi };
            const spec = props.group.spec;
    ...
    class SpreadsheetHeader extends BaseComponent {
        ...
        render() {
            ...
            const renderProps = { view: this.context.viewApi };
            const rowNodes = [];
            rowInnerHeights = rowInnerHeights.slice(); // copy, because we're gonna pop
            if (superHeaderRendering) {
                const rowInnerHeight = rowInnerHeights.shift();
            ...
            rowNodes.push(y("tr", { key: "row", role: "row" }, colSpecs.map((colSpec, i) => {
                const isLastCol = i === (colSpecs.length - 1);
        ...
        _handleColResizerEl(resizerEl, index) {
            const { colDraggings } = this;
            if (!resizerEl) {
                const dragging = colDraggings[index];
                if (dragging) {
                    dragging.destroy();
                ...
                const dragging = this.initColResizing(resizerEl, parseInt(index, 10));
                if (dragging) {
        ...
        initColResizing(resizerEl, index) {
            const { pluginHooks, isRtl } = this.context;
            const { onColWidthChange } = this.props;
            const ElementDraggingImpl = pluginHooks.elementDraggingImpl;
            if (ElementDraggingImpl) {
                const dragging = new ElementDraggingImpl(resizerEl);
                let startWidth; // of just the single column
                let currentWidths; // of all columns
                dragging.emitter.on('dragstart', () => {
                    const allCells = findElements(elementClosest(resizerEl, 'tr'), 'th');
    ...
    class ResourceTimelineLane extends BaseComponent {
        ...
        render() {
            const { props, context } = this;
            const { options } = context;
            const renderProps = this.refineRenderProps({ resource: props.resource, context });
    ...
    */
    ...
    class ResourceTimelineLanesBody extends BaseComponent {
        render() {
            const { props, context } = this;
            const { rowElRefs, innerHeights } = props;
    ...
    class ResourceTimelineGrid extends DateComponent {
        ...
        render() {
            ...
            const { dateProfile, tDateProfile } = props;
            const timerUnit = greatestDurationDenominator(tDateProfile.slotDuration).unit;
            const hasResourceBusinessHours = this.computeHasResourceBusinessHours(props.rowNodes);
            const splitProps = this.resourceSplitter.splitProps(props);
            const bgLaneProps = splitProps[''];
            const bgSlicedProps = this.bgSlicer.sliceProps(bgLaneProps, dateProfile, tDateProfile.isTimeScale ? null : props.nextDayThreshold, context, // wish we didn't need to pass in the rest of these args...
            dateProfile, context.dateProfileGenerator, tDateProfile, context.dateEnv);
            // WORKAROUND: make ignore slatCoords when out of sync with dateProfile
            const slatCoords = state.slatCoords && state.slatCoords.dateProfile === props.dateProfile ? state.slatCoords : null;
        ...
        // ------------------------------------------------------------------------------------------
        queryHit(positionLeft, positionTop) {
            const rowCoords = this.rowCoords;
            const rowIndex = rowCoords.topToIndex(positionTop);
            if (rowIndex != null) {
                const resource = this.props.rowNodes[rowIndex].resource;
                if (resource) { // not a group
                    const slatHit = this.slatsRef.current.positionToHit(positionLeft);
                    if (slatHit) {
                        return {
                            dateProfile: this.props.dateProfile,
                            dateSpan: {
    ...
    class ResourceTimelineViewLayout extends BaseComponent {
        ...
        render() {
            ...
            const resourceAreaWidth = state.resourceAreaWidthOverride != null
        ...
        getResourceScroll() {
            const timeBodyScrollerEl = this.timeBodyScrollerElRef.current;
        ...
        initSpreadsheetResizing() {
            const { isRtl, pluginHooks } = this.context;
            const ElementDraggingImpl = pluginHooks.elementDraggingImpl;
            const spreadsheetHeadEl = this.spreadsheetHeaderChunkElRef.current;
            if (ElementDraggingImpl) {
                const rootEl = this.rootElRef.current;
                const dragging = this.spreadsheetResizerDragging = new ElementDraggingImpl(rootEl, '.fc-resource-timeline-divider');
                let dragStartWidth;
                let viewWidth;
                dragging.emitter.on('dragstart', () => {
    ...
    class ResourceTimelineView extends BaseComponent {
        constructor(props, context) {
            ...
            this.handleScrollRequest = (request) => {
                const { rowCoords } = this;
                const layout = this.layoutRef.current;
                const rowId = request.rowId || request.resourceId;
                if (rowCoords) {
                    if (rowId) {
                        const rowIdToIndex = this.buildRowIndex(this.renderedRowNodes);
                        const index = rowIdToIndex[rowId];
                        if (index != null) {
                            const scrollTop = (request.fromBottom != null ?
        ...
        render() {
            ...
            const { superHeaderRendering, groupSpecs, orderSpecs, isVGrouping, colSpecs } = this.processColOptions(context.options);
            const tDateProfile = this.buildTimelineDateProfile(props.dateProfile, context.dateEnv, options, context.dateProfileGenerator);
            const rowNodes = this.rowNodes = this.buildRowNodes(props.resourceStore, groupSpecs, orderSpecs, isVGrouping, props.resourceEntityExpansions, options.resourcesInitiallyExpanded);
            const { slotMinWidth } = options;
            const slatCols = buildSlatCols(tDateProfile, slotMinWidth || this.computeFallbackSlotMinWidth(tDateProfile));
            return (y(ViewContainer, { elClasses: [
        ...
        queryResourceScroll() {
            const { rowCoords, renderedRowNodes } = this;
            if (rowCoords) {
                const layout = this.layoutRef.current;
                const trBottoms = rowCoords.bottoms;
                const scrollTop = layout.getResourceScroll();
                const scroll = {};
                for (let i = 0; i < trBottoms.length; i += 1) {
                    const rowNode = renderedRowNodes[i];
                    const elBottom = trBottoms[i] - scrollTop; // from the top of the scroller
                    if (elBottom > 0) {
    ...
    function processColOptions(options) {
        const allColSpecs = options.resourceAreaColumns || [];
        ...
        const plainColSpecs = [];
        const groupColSpecs = []; // part of the colSpecs, but filtered out in order to put first
        let groupSpecs = [];
        let isVGrouping = false;
        for (const colSpec of allColSpecs) {
            if (colSpec.group) {
        ...
        const mainColSpec = plainColSpecs[0];
        ...
        const allOrderSpecs = options.resourceOrder || DEFAULT_RESOURCE_ORDER;
        const plainOrderSpecs = [];
        for (const orderSpec of allOrderSpecs) {
            let isGroup = false;
