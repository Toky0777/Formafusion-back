...
class ChiffreAffaireRepportingCfp extends Controller
{
    ...
    private function getCaByMonth($year)
    {
        ...
        $projectByMonth = DB::table('v_union_projets')
            ->select(
                DB::raw('MONTH(dateDebut) as month_value'),
    ...
    public function caMonthYearSimple($year)
    {
        $monthlyData = $this->getMonthlyDataSimple($year);

        return response()->json([
            'data' => $monthlyData,
            'total_price' => $this->getTotalPriceProject($year),
            'total_projects' => count($monthlyData)
        ]);
    }

    public function caMonthYearExport($year)
    {
        $data = $this->getMonthlyDataSimple($year);

        // Si c'est une requête AJAX, retourner les données
        ...
        // Sinon, exporter en Excel
        return $this->exportToExcel($data, $year);
    }

    private function getMonthlyDataSimple($year)
    {
        $idProjectSubContractors = DB::table('project_sub_contracts')
            ->where('idSubContractor', Customer::idCustomer())
            ->pluck('idProjet');

        // Projets sans sous-traitant
        $projectsNoSubContractor = DB::table('v_union_projets')
            ->select(
                'idProjet',
                'module_name',
        ...
        $projectSubContractors = DB::table('v_union_projets')
            ->select(
                ...
                'dateDebut',
                'dateFin',
                DB::raw('total_ht_sub_contractor as total_ttc'),
                'project_reference',
                'etp_name',
                DB::raw('MONTH(dateDebut) as month_value')
            )
        ...
        foreach ($projects as $project) {
            $month = (int)$project->month_value;
            $monthTotals[$month] += $project->total_ttc;

            $results[] = [
                'month_name' => $this->getMonthName($month),
                'month_value' => $month,
                'project_reference' => $project->project_reference ?? 'N/A',
                'moduleName' => $project->module_name,
                'etpName' => $project->etp_name ?? 'Pas de client',
                'dateDebut' => Carbon::parse($project->dateDebut)->format('d/m/Y'),
                'dateFin' => Carbon::parse($project->dateFin)->format('d/m/Y'),
                'total_ttc' => $project->total_ttc,
                'total_ttc_formatted' => number_format($project->total_ttc, 0, ',', ' ') . ' Ar'
            ];
        }
        ...
        return [
            ...
            'month_totals_formatted' => array_map(function ($total) {
                return number_format($total, 0, ',', ' ') . ' Ar';
            }, $monthTotals)
        ];
    }
    ...
    private function exportToExcel($data, $year)
    {
        ...
        $filename = "CA_Mensuel_Simplifie_{$year}_" . date('d-m-Y') . ".csv";
