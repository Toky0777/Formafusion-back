...
use NumberToWords\Legacy\Numbers\Words\Locale\Tr;

class QcmInvitCfpCampController extends Controller
{
 ...
 */
    public function index_campaign()
    {
        ...
        // Transform campaigns to include payment details
        $campaignsWithPaymentDetails = $campaigns->map(function ($campaign) {
            $paymentDetails = $this->getCreditsForCampaign($campaign->idInvitCamp);
            
            // Convert campaign to array and add payment details
            $campaignData = $campaign->toArray();
            
            // Add each payment detail field to the main campaign object
        ...
        return response()->json([
            ...
            'campaigns' => $campaignsWithPaymentDetails,
            'user' => $user,
        ]);
    }
    ...
    public function storeStepOne(Request $request)
    {
        ...
        return response()->json([
            'status' => 'success',
            'nextRoute' => route('qcm.invitation.cfp.campaign.step-two')
        ]);
    }
    ...
    {
        $qcms = Qcm::where('user_id', Auth::user()->id)->where('statut', 1)->get();
    ...
    public function storeStepTwo(Request $request)
    {
        ...
        return response()->json([
            'status' => 'success',
            'nextRoute' => route('qcm.invitation.cfp.campaign.step-three')
        ]);
    }
    ...
    public function stepThree()
    {
        ...
        $user = Auth::user();
        if ($user->hasRole("Cfp")) {
            $employees = DB::table('v_apprenant_union')->where('idCfp', $user->id)->get();
            $selectedEmployees = Session::get('campaign_data.employee_ids', []);
            $etpname = "";
            $etp = [];
            //cherche les entreprises origine des apprenants
            foreach ($employees as $employee) {
                if ($employee->etp_name !== $etpname) {
                    if ($etpname != "") {
                        array_push($etp, ["id" => $employee->idEtp, "etpname" => $employee->etp_name]);
                    }
                    $etpname = $employee->etp_name;
                }
            }
            //----------------------------------------------
            return response()->json([
        ...
        else if($user->hasRole("Formateur")) {
            $cfp = DB::table('cfp_formateurs')->where('idFormateur', $user->id)->first();
            $employees = DB::table('v_apprenant_union')->where('idCfp', $cfp->idCfp)->get();
            $selectedEmployees = Session::get('campaign_data.employee_ids', []);
            return response()->json([
                'extends_containt' => $this->determineLayoutBasedOnRole(),
                'employees' => $employees,
    ...
    public function ajaxUpdateEmployees(Request $request)
    {
        ...
        return response()->json([
            'status' => 'success',
            'nextRoute' => route('qcm.invitation.cfp.campaign.step-four')
        ]); 
    }
    ...
    public function createCampaign(Request $request)
    {
        ...
        $campaignData = [
            ...
            'qcm_id' => $request->input('idQCM'),
            'employee_ids' => $request->input('selectedEmployees')
        ];
        ...
        try {
            ...
            return response()->json([
                ...
                'campaign' => $request->input('selectedEmployees')
            ]);
        } catch (\Exception $e) {
            // Log the error
    ...
    private function processInvitations($campaign, $employeeIds, $qcm, $enterpriseId, $validationData)
    {
        ...
        foreach ($employees as $employee) {
            if ($employee->emp_email) {
                $employee->email = $employee->emp_email;
                $employee->name = $employee->emp_name;
                $employee->firstName = $employee->emp_firstname;
            }
