...
class SkillMatrixEtpController extends Controller
{
    ...
    {
        $projectIntras = DB::table('v_skill_intras')
            ->select('idProjet', 'idModule', 'module_name', 'module_description', 'module_image', 'start_date', 'end_date', 'project_status', 'cfp_name', 'cfp_logo')
            ->where('idEtp', Customer::idCustomer())
            ->whereIn('project_status', ["Terminé", "Cloturé", "En cours"])
            ->orderBy('project_name', 'asc');

        $projects = DB::table('v_skill_inters')
            ->select('idProjet', 'idModule', 'module_name', 'module_description', 'module_image', 'start_date', 'end_date', 'project_status', 'cfp_name', 'cfp_logo')
            ->where('idEtp', Customer::idCustomer())
            ->whereIn('project_status', ["Terminé", "Cloturé", "En cours"])
            ->orderBy('project_name', 'asc')
    ...
    public function show($idProjet)
    {
        $project = DB::table('projets as p')
            ->select('p.idProjet', 'p.idModule', 'm.moduleName as module_name', 'p.project_reference', 'p.dateDebut as start_date', 'p.dateFin as end_date', 'c.customerName as cfp_name')
