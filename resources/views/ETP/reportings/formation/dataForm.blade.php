...
@section('content')
    <div class="flex flex-col w-full p-4 mx-auto mt-4 xl:p-0 gap-y-4 xl:container align-center">
        <div class="flex flex-row gap-3">
            <x-menu-reporting click='formation' />
            <x-menu-historique click='historique' />
            <x-menu-client click='client' />
            <x-menu-cours click='cours' />
        </div>
        <form action="{{ route('reporting.filter.formation') }}" method="post" class="">
            <div class="grid grid-cols-4 gap-4 p-4 bg-white rounded-lg shadow-sm">
                ...
                <span class="inline-flex grid items-center grid-cols-2 col-span-3 gap-4 grid-cols-subgrid">
                    <label class="input input-bordered flex items-center text-slate-400 gap-2">
                        Plage de date
                        <input type="text" id="daterange" name="daterange" class="grow text-slate-600" placeholder="" />
                    </label>

                    <select id="formation" name="formation" class="select select-bordered w-full text-slate-600">
